...
Copyright © 2004-2011, 2012-2015 Daniel Burrows, Manuel A. Fernandez Montecelo
...
Tabla de contenidos
  ...
  I. Referencia de la línea de órdenes
        ...
        aptitude-run-state-bundle — desempaqueta un fichero de estado de
        aptitude e invoca aptitude sobre éste
...
Lista de ejemplos
  ...
  2.9. Uso de pattern (patrón) para agrupar paquetes en base a su desarrollador.
...
  “Maestro, ¿posee Emacs la naturaleza del Buda?” preguntó el novicio.                  
   ...
   “No veo porqué no,” respondió el maestro. “Va sobrado en todo lo demás.” Varios años
   después, de súbito, el novicio alcanzó la iluminación.

                                                                                  -- John Fouhy


¡Hola y bienvenido al Manual de usuario de aptitude! Esta sección introductoria
explica qué es aptitude y como conseguirlo; en lo referente a información de su
uso, consulte Capítulo 1, Empezar.
...
Un gestor de paquetes mantiene un registro del software que está instalado en su
ordenador, y le permite instalar software nuevo, actualizarlo a versiones más
recientes, o eliminar software de una manera sencilla. Como su propio nombre
sugiere, los gestores de paquetes gestionan paquetes: conjuntos de ficheros que
se agrupan y que puede instalar y eliminar como conjunto.
...
pequeños y relacionados entre si se encuentren en el mismo paquete: por ejemplo,
el paquete fileutils contiene varias órdenes de Unix, tales como ls, cp, etc.
...
* Si un paquete A depende de otro paquete B, entonces B es necesario para que A
  ...
  data para permitir que el editor gráfico GIMP pueda acceder a sus ficheros
  críticos de datos.
...
* Si un paquete A recomienda otro paquete B, entonces B ofrece una importante
  ...
  para que Mozilla funcione, la mayoría de usuarios desearán que Mozilla permita
  la transmisión de datos de manera confidencial (tales como los números de una
  tarjeta de crédito).
...
realiza esta función (conocido como dpkg) hace exactamente esto y nada más. Esto
es suficiente si se descarga uno o dos paquetes a mano, pero enseguida se
...
si después decide eliminar el ya obsoleto software, esos paquetes adicionales se
quedarían en el sistema consumiendo espacio a menos que los elimine manualmente.
...
de toda esta labor por Ud. apt proporciona una base común sobre la que construir
estos programas: además de aptitude, programas tales como synaptic y apt-watch
hacen uso de apt.

apt funciona mediante el registro de una lista de los paquetes que se pueden
descargar desde Debian a su ordenador. Esta lista es útil a la hora de encontrar
los paquetes a actualizar y para instalar paquetes nuevos. apt también puede
resolver problemas de dependencias automáticamente: por ejemplo, cuando escoja
instalar un paquete, encontrará cualquier paquete adicional requerido e
instalará esos también.

Cuando use un gestor de paquetes basado en apt, tales como aptitude, por lo
general realizará tres tareas básicas: actualizar la lista de paquetes que están
disponibles mediante la descarga de listas nuevas desde los servidores de
Debian, seleccionar qué paquetes se deberían instalar, actualizar o eliminar, y
finalmente confirmar sus selecciones llevando a cabo las instalaciones,
eliminaciones, etc.

Los sistemas de gestión de paquetes basados en apt leen la lista de “fuentes”
(«sources», repositorios de paquetes para Debian) del fichero /etc/apt/
sources.list. El formato y contenido de este fichero están más allá del alcance
de este documento, pero se describen en la página de manual sources.list(5).
...
sencilla y común de instalar aptitude. Solo se debe intentar una instalación
desde las fuentes si los paquetes binarios no están disponibles por alguna
razón, o si tiene necesidades especiales que no cubren los paquetes binarios.
...
También puede construir aptitude desde las fuentes; por otro lado, este no es un
ejercicio útil a menos que tenga apt ya instalado en su sistema. Si lo está,
puede instalar aptitude desde las fuentes mediante los siguientes pasos:
  ...
  1. Instalar los siguientes programas:
     ...
     o Los ficheros de desarrollo de apt, que generalmente se encuentran en un
       paquete con un nombre parecido a libapt-pkg-dev.

     o La biblioteca libsigc++-2.0, disponible en el paquete libsigc++-2.0-dev o
       desde http://libsigc.sourceforge.net.

     o La biblioteca cwidget, disponible en el paquete libcwidget-dev o en http:
       //cwidget.alioth.debian.org.

     o El programa gettext, que debería estar incluido en su distribución de
       Linux.
  ...
  2. Por último pero no por ello menos importante, descargue la versión más
     ...
     unstable/admin/aptitude (desplácese hasta la base de la página y descargue
     el fichero “.orig.tar.gz”).


Una vez que disponga de todos los componentes necesarios, abra una terminal y
ejecute la orden tar zxf aptitude-0.7.4.tar.gz para desempaquetar el código
fuente. Una vez que haya desempaquetado el código fuente, introduzca cd
aptitude-0.7.4 && ./configure && make para compilar aptitude. Si tiene éxito,
...
Si quiere probar el código fuente más reciente de aptitude, puede descargar el
código fuente no publicado de aptitude utilizando Git. Instale Git (disponible
en http://git-scm.com/) y ejecute la orden git clone git://anonscm.debian.org/
aptitude/aptitude.git para obtener el código fuente más reciente.


IMG.alt.suffix Aviso
 = \[Aviso]]
               El repositorio Git de aptitude es un árbol de desarrollo activo; varía a medida
               que se corrigen fallos y nuevas características son añadidas, y no hay ninguna
               garantía de que pueda compilarlo, ¡no digamos ejecutarlo correctamente! Los
               informes de error son bienvenidos, pero sea consciente de que utiliza código en
...
devel@lists.alioth.debian.org>. Los ficheros de esta lista se encuentra en http:
...
<aptitude-devel@lists.alioth.debian.org>. Pero si prefiere mandarlos a través de
un correo privado, debe hacerlo a <aptitude@packages.debian.org> o a
<dburrows@debian.org>. Apreciaríamos de manera especial una breve descripción de
la motivación que hay detrás de su parche, y una explicación de su
funcionamiento.
...
fuente esté disponible en su ordenador (consulte la sección anterior), puede
utilizar cd para acceder a la carpeta y ejecutar git pull para actualizarlo con
cualquier cambio que se haya podido introducir en el repositorio central.

Para recibir notificaciones automáticas cuando se realicen cambios en el código
base de aptitude, suscríbase a la fuente web Atom disponible en http://
anonscm.debian.org/gitweb/?p=aptitude/aptitude.git;a=atom o a la fuente web RSS
disponible en http://anonscm.debian.org/gitweb/?p=aptitude/aptitude.git;a=rss.


Compilar aptitude desde el árbol de desarrollo

Para compilar aptitude desde el repositorio Git, debe tener instalados los
programas autoconf y automake. Introduzca sh ./autogen.sh && ./configure para
generar los ficheros necesarios para compilar aptitude, a continuación ejecute
make y make install.
...
aptitude (consulte Capítulo 2, Guía de referencia de aptitude para ello), pero
...
Una vez que el almacén se cargue (esto puede llevar algún tiempo en máquinas más
lentas), debería aparecer la pantalla central de aptitude:
...
Como puede ver, la pantalla principal de aptitude está dividida en varias áreas.
La línea azul en la parte superior de la terminal es la barra de menú, y las
líneas inferiores muestran mensajes informativos que describen algunas órdenes
importantes. El espacio negro a continuación es la lista de todos los paquetes
disponibles, en la cual aparecen algunos grupos de paquetes. El grupo
seleccionado (“Paquetes instalados”) está resaltado, y su descripción se muestra
en el espacio inferior en negro.

Como sugiere la línea superior de la pantalla, puede acceder al menú de aptitude
pulsando Control+t; también puede pulsar sobre el titulo del menú si su sistema
lo permite. Pulse Control+t para abrir el menú Acciones:
...
sin seleccionar nada, pulse Control+t otra vez. Una descripción del elemento del
menú resaltado aparecerá en la base de la pantalla. Si un elemento del menú se
puede activar mediante un atajo de teclado, el atajo aparecerá en el menú: por
ejemplo, puede ejecutar la orden “Actualizar la lista de paquetes” pulsando u.
...
IMG.alt.suffix Nota
  = \[Nota]]
               aptitude ocultará automáticamente los grupos de paquetes que estén vacíos, así que
               puede que vea más o menos grupos de los que aparecen en esta imagen.


En la captura de pantalla anterior, el primer grupo (“Paquetes instalados”) está
resaltado, indicando que está seleccionado en ese momento. Puede navegar a
través de la lista con las flechas de dirección; observe que la descripción en
el campo inferior a la lista de paquetes varía a medida que navega. Para
“expandir” un grupo, pulse Intro cuando el grupo esté resaltado:
...
--\ Paquetes instalados
  ...
  --- admin - Utilidades de administración (instalación de programas, gestión de
usuarios, etc)
...
Como puede ver, el grupo “Paquetes instalados” se ha expandido para mostrar su
contenido: contiene un número de sub-grupos, definidos de manera abierta
dependiendo del tipo de software que contienen. Si expandimos la sección “admin”
resaltándolo y pulsando Intro, podemos ver:
...
El grupo “admin” contiene un solo sub-grupo, el archivo “principal” («main») de
Debian. ¡Si expande este grupo verá algunos paquetes!


IMG.alt.suffix Sugerencia
     = \
[Sugerencia]]  Para ahorrar tiempo, puede usar la tecla [ para expandir todos los sub-grupos de
               un grupo a la vez. Selecciona “Paquetes instalados” y pulsar [ revelaría
               inmediatamente los paquetes de la imagen inferior.
...
lista de paquetes mostrando una página de información cada vez usando las teclas
Repág y Avpág.


IMG.alt.suffix Sugerencia
     = \
[Sugerencia]]  Cuando hay más información en la zona inferior de la pantalla de la que cabe en
               el espacio disponible, puede emplear las teclas a y z para desplazarse a través
               de la información.
...
Para encontrar rápidamente un paquete cuyo nombre conoce, pulse / para abrir una
ventana de diálogo de búsqueda:
...
poderoso lenguaje de búsqueda de aptitude, descrito en “Patrones de búsqueda”.


IMG.alt.suffix Sugerencia
     = \
[Sugerencia]]  Puede buscar hacia atrás en la lista de paquetes si pulsa \, y puede repetir la
               ultima búsqueda si pulsa n tras cerrar la ventana de búsqueda.
...
Este dialogo funciona exactamente igual que el dialogo de búsqueda, sólo que en
vez de realzar el siguiente paquete que coincide con lo que ha introducido en el
cuadro de dialogo, oculta todos los paquetes que no lo hacen. Por ejemplo, si
...
IMG.alt.suffix Sugerencia
     = \
[Sugerencia]]  Sólo puede cambiar la configuración del sistema como el usuario root. Si quiere
               experimentar con aptitude, puede iniciarlo de manera segura como cualquier otro
               usuario aparte de como root sin dañar el sistema de ninguna manera. aptitude le
               avisará cuando esté intentando hacer algo que sólo el usuario root puede hacer, y
               si desea continuar tendrá que introducir la contraseña de root.


Todos los cambios a un paquete se realizan primero, resaltándolo en la lista de
paquetes, y después pulsando la tecla correspondiente a la acción que se debería
realizar. Las teclas de acción básicas [2] son +, para instalar o actualizar un
paquete, - para eliminarlo y = para evitar que un paquete se actualice
automáticamente (esto se conoce como retener el paquete). Estas acciones no se
ejecutan inmediatamente; aptitude simplemente actualiza la lista de paquetes
para mostrar los cambios solicitados.

Por ejemplo, en la siguiente imagen se ha seleccionado el paquete kaffeine-
mozilla y la tecla +, pulsada. El paquete está ahora resaltado en verde, y ha
aparecido la letra “i” a la izquierda de su nombre para indicar que se va a
instalar; además, se muestra una estimación del espacio que el paquete ocupará
en el disco duro.
...
IMG.alt.suffix Sugerencia
     = \
[Sugerencia]]  En cualquier momento puede usar Deshacer → Deshacer (Control+u) para “deshacer”
               cualquier cambio realizado a uno o más paquetes. Esto es bastante útil en caso de que
               una acción tenga consecuencias inesperadas y desee “revertir” los paquetes a su
               estado anterior.
...
la pantalla aparece una barra con una sugerencia de cómo solucionar el problema.
Para encontrar con rapidez paquetes rotos en la lista de paquetes puede pulsar b
o realizar una búsqueda de ?broken.


IMG.alt.suffix Nota
  = \[Nota]]
               El texto [1(1)/...] indica el progreso del solucionador de dependencias de
               aptitude. El primer número es la solución que está seleccionada en ese momento, y
               el segundo es el numero de soluciones que aptitude ha generado. La presencia del
               texto “...” indica que pueden existir algunas soluciones adicionales más allá de
               las generadas; si aptitude estuviese seguro de haber generado la única solución
...
ejemplo, si pulsase «.» en la pantalla anterior, se le presentaría la siguiente
solución:
...
Además de las órdenes básicas disponibles cuando examina las soluciones, puede
pulsar r para “rechazar” las acciones que desapruebe. Por ejemplo, la primera
solución cancelaría la eliminación de sound-juicer ¡precisamente la acción que
intentaba ejecutar! Si pulsa r sobre el espacio correspondiente a esta acción,
le diría a aptitude que no debería cancelar la eliminación de sound-juicer de
esta manera.
...
la misma versión de sound-juicer se ha vuelto roja y marcada con una “R”,
indicando que se ha rechazado. Las soluciones que pudiese generar en el futuro
(esto es, cualquier solución que no haya examinado aún) no incluirían esta
acción, aunque seguirían disponibles las soluciones previamente generadas y que
contienen esta solución.


IMG.alt.suffix Nota
  = \[Nota]]
               En la captura de pantalla anterior, se puede ver una descripción de sound-
               juicer, el cual se muestra en el centro de la pantalla; bajo esta descripción
               puede ver la dependencia que causó que sound-juicer se haya mantenido en su
               versión actual, además de todas las maneras que aptitude conoce para resolver
               esta dependencia.
...
solución, omitiendo la solución que cancela la instalación de sound-juicer y que
desactualiza gstreamer0.8-cdparanoia.
...
una vez más la acción rechazada y pulsa r; esto permitiría que se generen otra
vez las soluciones que contienen la acción rechazada, incluyendo cualquier
solución que haya “omitido” con anterioridad.

La contrario a rechazar una acción es aprobarla. Para aprobar una acción
simplemente seleccione la acción y presione a; esto forzaría al solucionador de
problemas a escoger esta acción cuando sea posible[3]. Las acciones aprobadas se
volverán verdes y se marcarán con “A”, como puede ver en la siguiente imagen:
...
IMG.alt.suffix Importante
     = \
[Importante]]  Si no resuelve ninguna dependencia rota, aptitude llevará a cabo su sugerencia
               actual automáticamente cuando confirme al pulsar g las selecciones que haya
               hecho. Por otro lado, es difícil resolver automáticamente los problemas de
               dependencias, y puede que no le guste el resultado final. Por ello, es siempre
               mejor observar lo que aptitude ha planeado hacer antes de llevar a cabo los
               cambios.
...
Si selecciona un paquete, en este espacio aparecerá una explicación de su estado
actual.
...
Además de la interfaz “gráfica” descrita en la sección anterior, también puede
usar aptitude desde la línea de órdenes de la misma manera que usaría apt-get.
Esta sección cubre las órdenes en línea de órdenes más comunes; para más
información, consulte la guia de referencia en la línea de órdenes de aptitude.
...
generales, consisten de nombres de paquetes y de diferentes opciones en la línea
de órdenes[4].
...
Las acciones más importantes son:
  ...
  aptitude full-upgrade
      ...
      IMG.alt.suffix Nota
        = \[Nota]]
                     Por razones históricas, esta orden se llamaba originalmente dist-upgrade,
                     y aptitude aún reconoce este nombre.
...
parámetro -s, que simboliza “simular”. Cuando se introduce -s en la línea de
órdenes el programa realiza todas las acciones que haría normalmente, pero en la
práctica no descarga, instala o elimina ningún fichero.
...
Además de las obvias opciones “Yes” y “No”, dispone de un número de órdenes que
puede usar para cambiar la información mostrada, o para definir futuras
acciones. Por ejemplo, pulsar s muestra u oculta información acerca del espacio
que cada paquete usará:
...
aptitude le preguntará qué hacer si su petición rompe dependencias de una manera
que no se pueda resolver de una forma sencilla:
...
Pulsar y (o simplemente Intro) aceptará la solución propuesta. Si pulsa n verá
...
Si pulsa q, cancelaría el solucionador automático y le permitiría resolver las
...
dependencias rotas (pulse ? para una lista completa de las órdenes disponibles).
Pulse n o Intro para salir de aptitude:

¿Desea resolver las dependencias manualmente? [N/+/-/_/:/?] n
Cancela.

Para una completa documentación acerca de las características de aptitude en la
línea de órdenes, consulte Referencia de la línea de órdenes.



--------------------------------------------------------------------------------
[2] También puede cambiar el estado de los paquetes usando el menú Paquete;
consulte “El menú Paquete” para más detalles.
...
[4]Una “opción” es una letra precedida de un guión: por ejemplo, “-a”, “-v”,
etc.

[5]Purgar un paquete elimina el paquete, así como también sus ficheros de
configuración.
...
  El Conejo Blanco se puso las gafas. 'Por favor, ¿por donde podría yo empezar,  
...
difícil recordar como hacer alguna operación, o incluso si es posible. De hecho,
muchas de las peticiones que llegan al autor acerca de la implementación de más
características describen características ya presentes, pero difíciles de
encontrar.[7]

En un intento de combatir esta oscuridad, esta guía de referencia describe todas
las capacidades y parámetros de configuración de aptitude, consulte Capítulo 1,
Empezar.


IMG.alt.suffix Nota
  = \[Nota]]
               Puede configurar el comportamiento y la apariencia de aptitude de varias
               maneras. Este manual describe el funcionamiento del programa con las
               opciones predeterminadas; las descripciones de las múltiples opciones que
               afectan al comportamiento se detallan en “Personalizar aptitude”.



La interfaz de usuario de aptitude en la terminal

Esta sección describe las partes que componen la interfaz de usuario de aptitude
en la terminal no relacionadas con gestionar paquetes.
...
directo”: números o letras que se pueden emplear para seleccionar la entrada
mientras el menú está activo. Estos atajos se muestran en un color mas claro que
el del resto del menú.

Además, algunos elementos del menú poseen “atajos”: combinaciones de teclas que
ejecutan la misma acción que la entrada de menú mientras no está activo. Dispone
de una lista de estos atajos en el lado derecho del menú.

De aquí en adelante, las órdenes de menú se escribirán así:Menú → Entrada
(tecla). Esto indica que debería escoger la Entrada del Menú menú, y esa tecla/
s es el atajo para esa orden.
...
|                                                |Si no hay una previsualización disponible, muestra una; de no ser así, ejecuta un|
|Acciones → Instalar/eliminar paquetes (g)     |proceso de instalación como se describe en “Descargar, instalar y eliminar      |
|                                                |paquetes.”.                                                                      |
|________________________________________________|___________________________________________________________________________________|
|Acciones → Actualizar la lista de paquetes (u)|Actualiza la lista de paquetes.                                                    |
|________________________________________________|___________________________________________________________________________________|
|Acciones → Marcar actualizable (U)            |Marca todos los paquetes actualizables, excepto aquellos que estén prohibidos o   |
|                                                |bloqueados para su actualización.                                                 |
|________________________________________________|___________________________________________________________________________________|
|Acciones → Olvidar paquetes nuevos (f)        |Descarta toda la información referente a qué paquetes son “nuevos” (vacía el|árbol
|                                                |“Paquetes nuevos”).                                                            |
|________________________________________________|___________________________________________________________________________________|
|                                                |Cancela toda instalación, eliminación, actualización y retención pendiente. Es |l
|Acciones → Cancelar acciones pendientes       |equivalente a ejecutar la orden Keep sobre cada paquete en la base de datos de     |
|                                                |paquetes.                                                                          |
|________________________________________________|___________________________________________________________________________________|
|Actions → Limpiar el almacén de paquetes     |Elimina todos los paquetes comprimidos que aptitude ha descargado [a].             |
|________________________________________________|___________________________________________________________________________________|
|                                                |Elimina cualquier paquete comprimido que aptitude descargó [a] y que ya no están |
|Acciones → Limpiar ficheros obsoletos         |disponibles. Se asume que estos paquetes ya están obsoletos, y se pueden eliminar |
|                                                |del disco duro sin precisar de una descarga que por otro lado sería inútil.      |
|________________________________________________|___________________________________________________________________________________|
|Acciones → Jugar al buscaminas                |Jugar al Buscaminas, como se describe en “Jugar al Buscaminas”.                |
|________________________________________________|___________________________________________________________________________________|
|Acciones → Convertirse en administrador       |Continua trabajando como el usuario root, consulte “Convertirse en root.”.     |
|________________________________________________|___________________________________________________________________________________|
|Acciones → Salir (Q)                          |Cierra aptitude guardando cualquier cambio hecho al estado de los paquetes.        |
|________________________________________________|___________________________________________________________________________________|
...
|                                 |Cancela el efecto del ultimo cambio realizado al estado de un paquete hasta  |
|Deshacer → Deshacer (Control+u)|el punto en que aptitude se inició, la lista de paquetes se ha actualizado o|
|                                 |en el que un proceso de instalación se llevó a cabo.                       |
|_________________________________|_____________________________________________________________________________|
...
|Paquete → Instalar (+)                         |Marca el paquete seleccionado para su instalación.                             |
|_________________________________________________|________________________________________________________________________________|
|Package → Reinstall (L)                        |Flag the currently selected package for reinstallation.                         |
|_________________________________________________|________________________________________________________________________________|
|Paquete → Eliminar (-)                         |Marca el paquete seleccionado para su eliminación.                             |
|_________________________________________________|________________________________________________________________________________|
|Paquete → Purgar (_)                           |Marca el paquete seleccionado para que sea purgado.                             |
|_________________________________________________|________________________________________________________________________________|
|                                                 |Cancela toda instalación, actualización o eliminación pendiente de ejecución|
|Paquete → Mantener (:)                         |sobre el paquete seleccionado, y elimina cualquier retención impuesta al       |
|                                                 |paquete.                                                                        |
|_________________________________________________|________________________________________________________________________________|
|Paquete → Retener (=)                          |Retener el paquete seleccionado.                                                |
|_________________________________________________|________________________________________________________________________________|
|Paquete → Marcar automático (M)               |Marca al paquete instalado como “automáticamente instalado”. Para más     |
|                                                 |información, consulte “Gestionar paquetes automáticamente instalados.”.   |
|_________________________________________________|________________________________________________________________________________|
|                                                 |Marca el paquete seleccionado como “manualmente instalado”. Para más inform|ción
|Paquete → Marcar manual (m)                    |referente a paquetes manual y automáticamente instalados, consulte “Gestionar|
|                                                 |paquetes automáticamente instalados.”.                                       |
|_________________________________________________|________________________________________________________________________________|
|                                                 |Si se selecciona un paquete susceptible de ser actualizado, prohíbe que se     |
|Paquete → Prohibir versiones (F)               |actualice a la versión disponible en el servidor. Si selecciona la versión de |n
|                                                 |paquete, prohíbe que el paquete se actualice a esa versión.                   |
|_________________________________________________|________________________________________________________________________________|
|                                                 |Muestra una pantalla que contiene información acerca del paquete seleccionado, |
|Paquete → Información (enter)                 |tales como de qué paquetes depende, así como qué paquetes dependen de él, y |as
|                                                 |diferentes versiones disponibles.                                               |
|_________________________________________________|________________________________________________________________________________|
|                                                 |Cuando explora la lista de paquetes, varía la información que puede ver en el |
|                                                 |área de información (la mitad inferior de la pantalla). El área de informaci�|n
|Paquete → Recorrer información de paquetes (i)|puede mostrar una larga descripción del paquete seleccionado (predeterminado), |
|                                                 |un resumen de las dependencias relacionadas con ese paquete o un análisis de qu|�
|                                                 |otros paquetes requieren o sugieren el paquete seleccionado.                    |
|_________________________________________________|________________________________________________________________________________|
|                                                 |Muestra el registro de cambios de Debian del paquete seleccionado. Para ver el  |
|Paquete → Registro de cambios (C)              |registro de cambios de una versión en particular, seleccione la versión y     |
|                                                 |ejecute esta orden.                                                             |
|_________________________________________________|________________________________________________________________________________|
...
|Solucionador → Examinar solución (e)  |Muestra una detallada descripción de la acción sugerida por el solucionador  |
|                                         |(consulte “Resolver dependencias de manera interactiva.”).                 |
|_________________________________________|_______________________________________________________________________________|
|Solucionador → Aplicar Solución (!)   |Realiza las acciones sugeridas por el solucionador.                            |
|_________________________________________|_______________________________________________________________________________|
|Solucionador → Solución siguiente (.) |Selecciona la siguiente sugerencia del solucionador.                           |
|_________________________________________|_______________________________________________________________________________|
|Solucionador → Solución anterior (,)  |Selecciona la anterior sugerencia del solucionador.                            |
|_________________________________________|_______________________________________________________________________________|
|Solucionador → Primera solución (<)   |Selecciona la primera sugerencia del solucionador.                             |
|_________________________________________|_______________________________________________________________________________|
|Solucionador → Última solución (>)   |Selecciona la última solución generada por el solucionador (consulte “Resol|er
|                                         |dependencias de manera interactiva.”).                                       |
|_________________________________________|_______________________________________________________________________________|
|                                         |Cuando examina una solución, conmuta si rechaza o no la acción seleccionada, |
|Solucionador → Conmutar Rechazados (r) |pasando después a la siguiente acción (consulte “Resolver dependencias de  |
|                                         |manera interactiva.”). Si la acción ya se aprobó, queda cancelada.         |
|_________________________________________|_______________________________________________________________________________|
|                                         |Cuando examina una solución, conmuta si la acción seleccionada se aprueba o n|,
|Solucionador → Conmutar Aceptada (a)   |y pasa a la siguiente acción (consulte “Resolver dependencias de manera     |
|                                         |interactiva.”). Si la acción ya se rechazó, esta queda cancelada.          |
|_________________________________________|_______________________________________________________________________________|
|                                         |Cuando examina una solución, muestra información detallada acerca del paquete|
|Solucionador → Ver objetivo (enter)    |afectado por la acción seleccionada (consulte “Resolver dependencias de mane|a
|                                         |interactiva.”).                                                              |
|_________________________________________|_______________________________________________________________________________|
|                                         |Rechaza (al igual que con Solucionador → Conmutar Rechazados (r)) todas las  |
|Solucionador → Rechazar romper bloqueos|acciones que rompen una retención o que instalan una versión prohibida. De   |
|                                         |manera predeterminada, estas acciones se rechazan a menos que defina Aptitude::|
|                                         |ProblemResolver::Allow-Break-Holds como true.                                  |
|_________________________________________|_______________________________________________________________________________|
...
|Buscar → Buscar (/)                      |Busca el siguiente paquete en la lista de paquetes que coincida con el patrón |
|                                           |de búsqueda (consulte “Patrones de búsqueda”).                           |
|___________________________________________|_______________________________________________________________________________|
|Buscar → Buscar hacia atrás (\)         |Busca el paquete anterior en la lista de paquetes que coincida con el patrón d|
|                                           |búsqueda (consulte “Patrones de búsqueda”).                              |
|___________________________________________|_______________________________________________________________________________|
|Buscar → Buscar otra vez (n)             |Repite la última búsqueda.                                                   |
|___________________________________________|_______________________________________________________________________________|
|                                           |Repite la última búsqueda, pero en dirección opuesta. Si la última orden de|
|Buscar → Buscar de nuevo hacia atrás (N)|búsqueda empleada fue «Buscar hacia atrás», esta orden ejecutaría una bús|ueda
|                                           |hacia delante, y viceversa.                                                    |
|___________________________________________|_______________________________________________________________________________|
|Buscar → Limitar vista (l)               |Filtra la lista de paquetes eliminando cualquier paquete que no coincida con el|
|                                           |patrón de búsqueda (consulte “Patrones de búsqueda”).                   |
|___________________________________________|_______________________________________________________________________________|
|Buscar → No limitar vista                |Elimina el filtro de la lista actual de paquetes (todos los paquetes serán    |
|                                           |visibles).                                                                     |
|___________________________________________|_______________________________________________________________________________|
|Buscar → Buscar Roto (b)                 |Busca el siguiente paquete roto. Esto equivale a buscar ?broken.               |
|___________________________________________|_______________________________________________________________________________|
...
|                              |Abre una vista de nivel superior en la cual puede modificar los parámetros de  |
|                              |aptitude. Las opciones de configuración se organizan en árbol, similar al árb|l
|Opciones → Preferencias     |de paquetes; para activar o desactivar una opción, selecciónela y pulse Espaci|
|                              |o Intro. Las opciones de configuración se guardan en ~/.aptitude/config en el  |
|                              |mismo momento de su selección.                                                 |
|______________________________|________________________________________________________________________________|
|Opciones → Deshacer opciones|Devuelve todas las opciones a sus valores predeterminados.                      |
|______________________________|________________________________________________________________________________|
...
IMG.alt.suffix Nota
  = \[Nota]]
               Para una introducción al funcionamiento de las vistas, consulte “Trabajar con
               varias vistas.”.
...
|________________________________________|______________________________________________________________________________|
|Vistas → Nueva vista de paquetes      |Crea una nueva vista de la lista de paquetes.                                 |
|________________________________________|______________________________________________________________________________|
|Vistas → Auditar Recomendaciones      |Crea una vista que muestra paquetes no instalados, recomendados por algún    |
|                                        |paquete instalado en su sistema.                                              |
|________________________________________|______________________________________________________________________________|
|Vistas → Nueva vista de paquetes plana|Crea una nueva vista de paquetes en la cual los paquetes no están            |
|                                        |categorizados.                                                                |
|________________________________________|______________________________________________________________________________|
|Vistas → Nuevo navegador debtags      |Crea una nueva vista de paquetes en la cual los paquetes están categorizados |
|                                        |según sus entradas de debtags.                                               |
|________________________________________|______________________________________________________________________________|
|Elementos adicionales                   |Pueden aparecer un número de elementos adicionales del menú correspondientes|a
|                                        |la vista activa actual. Para cambiar a otra vista, selecciónela desde el men�|.
|________________________________________|______________________________________________________________________________|
...
|_____________________________|__________________________________________________________________________|
|Ayuda → Manual de usuario  |Muestra el Manual de usuario (este documento).                            |
|_____________________________|__________________________________________________________________________|
|Ayuda → PUF                |Muestra el PUF de aptitude.                                               |
|_____________________________|__________________________________________________________________________|
|Ayuda → Registro de cambios|Muestra un historial de los cambios más significativos hechos a aptitude.|
|_____________________________|__________________________________________________________________________|
|Ayuda → Licencia           |Muestra los términos bajo los cuales puede copiar, modificar y distribuir|
|                             |aptitude.                                                                 |
|_____________________________|__________________________________________________________________________|
...
aptitude permite trabajar con varias “vistas” a la vez. Una “vista” (a veces
llamada “pantalla”) es simplemente algo que puede aparecer en el área de la
pantalla por debajo de la barra de menú. La vista más común es la lista de
paquetes, aunque vistas de descargas también son habituales.
...
Puede cerrar la vista actual usando Vistas → Cierra (q). Para cambiar a la vista
anterior o a la siguiente, use Vistas → Siguiente (F6) y Vistas → Prev (F7), o
pulse sobre el nombre de la vista en la parte superior de la pantalla; también
puede ver una lista de todas las vistas activas en el menú Vistas.
...
new view using Vistas → Nueva vista de paquetes.
...
contraseña de administrador; una vez que la haya introducido aptitude realizará
la acción que requiere privilegios de root. Aun será root después de que la
acción finalice.

Puede identificarse como root en cualquier momento usando la orden Acciones →
Convertirse en administrador. Cualquier cambio realizado al estado de los
paquetes será preservado (pero no se guardarán hasta que quite aptitude).

De manera predeterminada, aptitude utilizará la orden su para obtener
privilegios de usuario root. Si desea usar otra orden, (como por ejemplo sudo),
defina la opción de configuración Aptitude::Get-Root-Command.
...
al día. Puede hacer esto usando la orden Acciones → Actualizar la lista de
paquetes (u).
...
paquete. Por ejemplo, el paquete webmin podría mostrar una sinopsis cercana a
ésta:
...
automáticamente seleccionado para su instalación (“A”), y también que no va
firmado (“U”). Puede ver en el lado derecho de la sinopsis la versión
actualmente instalada y la última versión disponible, así como una indicación de
cuanto espacio se va a usar al actualizar el paquete.


IMG.alt.suffix Sugerencia
     = \
[Sugerencia]]  Puede configurar cómo se muestran las sinopsis de los paquetes; consulte
               “Personalizar la presentación de los paquetes” para más detalles.
...
estado actual. El segundo carácter simboliza la acción que se ejecutará sobre el
paquete. El tercer carácter indica si el paquete ha sido automáticamente
instalado (consulte “Gestionar paquetes automáticamente instalados.”), y el
cuarto carácter indica si el paquete está firmado (consulte “Llaves GPG:
Entender y gestionar la confianza de los paquetes.”).

Los cuatro posibles valores de la marca de “estado actual” se tratan en
Figura 2.9, “Valores de la marca de “estado actual””, y los posibles valores de
“acción” se tratan en Figura 2.10, “Valores de la marca de “acción””.
...
p - el paquete y todos sus ficheros de configuración se eliminaron, o el paquete
    nunca se instaló.
...
h - el paquete se va a retener: permanecerá en su versión actual, aunque haya una
    nueva versión disponible, hasta que se cancele la retención .

F - Se prohibió la actualización del paquete.

r - el paquete se va a reinstalar.

    el paquete está “roto”: algunas de sus dependencias no serán satisfechas.
B - aptitude no le permitirá instalar, eliminar o actualizar nada mientras tenga
    paquetes rotos.
...
dependencias; de ser así se mostraría aquí información referente al problema de
dependencias. Para pasar de la información de dependencias a la descripción del
paquete, pulse i.
...
paquetes.
    Proporciona la herramienta apt-get y ún metodo dselect que ofrece una manera
más
...
IMG.alt.suffix Sugerencia
     = \
[Sugerencia]]  Para satisfacer rápidamente una dependencia, seleccione la dependencia y
               presione +; aptitude intentará satisfacerlas automáticamente por usted.
...
lista de paquetes y ejecute la orden. Estas órdenes se pueden aplicar también a
grupos de paquetes, seleccionando la cabecera del grupo (por ejemplo, “Paquetes
actualizables”), y ejecutando la orden.
...
|Instalar: Paquete → Instalar (+)                               |                                                                                  |
|                                                                 |Si el paquete no está instalado, se instalará. Si ya lo está, se actualizará, |e
|                                                                 |ser posible, y cualquier retención en efecto se cancelará.                      |
|_________________________________________________________________|__________________________________________________________________________________|
|                                                                 |Marcar el paquete seleccionado para su eliminación.                              |
|Eliminar: Paquete → Eliminar (-)                               |                                                                                  |
|                                                                 |Si el paquete está instalado, se eliminará.                                     |
|_________________________________________________________________|__________________________________________________________________________________|
|                                                                 |Marcar el paquete para ser purgado                                                |
|                                                                 |                                                                                  |
|Purgar: Paquete → Purgar (_)                                   |Si el paquete esta instalado, se eliminará. Mas aún, aunque se elimine, cualquie|
|                                                                 |fichero resultante (tales como los ficheros de configuración) relacionados con el|
|                                                                 |paquete también se eliminarán del sistema.                                      |
|_________________________________________________________________|__________________________________________________________________________________|
|                                                                 |Marcar el paquete para que se mantenga en su versión actual.                     |
|                                                                 |                                                                                  |
|Mantener: Paquete → Mantener (:)                               |Cualquier acción que se fuese a llevar a cabo sobre el paquete -- instalación,  |
|                                                                 |eliminación o actualización -- se cancela, y cualquier retención impuesta al   |
|                                                                 |paquete se elimina.                                                               |
|_________________________________________________________________|__________________________________________________________________________________|
|                                                                 |Imponer una retención al paquete.                                                |
|                                                                 |                                                                                  |
|Retener: Paquete → Retener (=)                                 |Al igual que con «Mantener», se cancela cualquier acción programada para el    |
|                                                                 |paquete. Además, el paquete no se actualizará automáticamente [a] hasta que    |
|                                                                 |elimine esta acción. Puede cancelar «Mantener» ejecutando la siguiente orden.  |
|_________________________________________________________________|__________________________________________________________________________________|
|                                                                 |El paquete no se actualizará automáticamente [a] a la versión a la que lo iba a|
|                                                                 |ser. Si se iba a actualizar, la actualización se cancela.                        |
|                                                                 |                                                                                  |
|                                                                 |Si ejecuta esta orden sobre una versión en particular de un paquete, el paquete n|
|Paquete → Prohibir versiones (F)                               |se actualizará a la versión escogida. Observe que sólo puede prohibir una versi|�n
|                                                                 |al mismo tiempo.                                                                  |
|                                                                 |                                                                                  |
|                                                                 |Esta funcionalidad se ha implementado en gran medida para la conveniencia de la   |
|                                                                 |distribución “unstable (sid)”, para que así se puedan evitar versiones de   |
|                                                                 |programas ya conocidas como malas.                                                |
|_________________________________________________________________|__________________________________________________________________________________|
|                                                                 |Flag the current package for reinstallation.                                      |
|                                                                 |                                                                                  |
|Reinstall: Package → Reinstall (L)                             |Tenga en cuenta que la reinstalación no se guardará cuando salga de aptitude o  |
|                                                                 |ejecute un proceso de instalación, por razones técnicas (básicamente, las capas|de
|                                                                 |software subyacentes, dpkg y apt no proporcionan ninguna manera de ver si una     |
|                                                                 |reinstalación ha tenido éxito o no).                                            |
|_________________________________________________________________|__________________________________________________________________________________|
|                                                                 |Define si un paquete se toma como automáticamente instalado; los paquetes        |
|Paquete → Marcar automático (M), Paquete → Marcar manual (m)|automáticamente instalados se eliminarán cuando ningún otro paquete dependa de |
|                                                                 |ellos. Para más información, consulte “Gestionar paquetes automáticamente    |
|                                                                 |instalados.”.                                                                   |
|_________________________________________________________________|__________________________________________________________________________________|
|[a] Ésto es, que no se verá afectado por Acciones → Marcar actualizable (U) o por las órdenes en línea de órdenes full-upgrade o safe-upgrade|
|____________________________________________________________________________________________________________________________________________________|

Además de estas órdenes que afectan al paquete seleccionado, hay dos órdenes que
afectan a un gran número de paquetes en una sola acción independientemente de lo
que haya seleccionado. Acciones → Olvidar paquetes nuevos (f) elimina el estado
“nuevo” de todos los paquetes de la lista de paquetes, y Acciones → Marcar
actualizable (U) marca todos los paquetes actualizables para su actualización,
...
Cambiar el estado de los paquetes como se describe en la sección anterior no
afecta de manera directa a lo que está instalado en el sistema. Por ello, puede
ajustar el estado de los paquetes sin afectar al sistema hasta que esté
satisfecho con lo que ve; una vez que lo esté, puede “confirmar” los cambios de
verdad, instalando y eliminando paquetes.[8]

Para confirmar los cambios, use la orden Acciones → Instalar/eliminar paquetes
(g). Seleccionar esta orden muestra una previsualización que describe los
cambios que se llevarán a cabo. Esta imagen es una simple lista de paquetes por
lo que puede manipular los paquetes (por ejemplo, cancelando eliminaciones no
deseadas) de la misma manera en que lo puede hacer en el lista principal.

Una vez que haya finalizado, use Vistas → Cierra (q) para cancelar la
instalación, o use Acciones → Instalar/eliminar paquetes (g) para proceder con
las selecciones. aptitude descargará en ese momento cualquier paquete necesario
para después continuar con la instalación.
...
infinitum. Para eliminar todos los ficheros de este directorio, use Actions →
Limpiar el almacén de paquetes; para eliminar sólo aquellos paquetes que ya no
se pueden descargar (p. ej., paquetes obsoletos), use Acciones → Limpiar
ficheros obsoletos.
...
de paquetes de José Hacker Aleatorio a su fichero sources.list para poder
...
en el sistema! Si estos paquetes tuviesen unos números de versión más elevados
que los que se encuentran el archivo de Debian, apt los instalaría sin dudar en
la siguiente actualización, permitiendo a Pepe realizar su sucia labor sin ser
detectado. Pepe podría incluso entrar en los servidores réplica de Debian y
reemplazar el software legítimo con su versión medicada.

Afortunadamente, las versiones más recientes de apt y de aptitude, tales como la
versión documentada en este manual, tienen defensas incorporadas para repeler
este tipo de ataques. apt utiliza unos fuertes mecanismos de seguridad basados
en el conocido software de cifrado GPG para verificar que los paquetes
...
IMG.alt.suffix Aviso
 = \[Aviso]]
               Los mecanismos de seguridad de apt proporcionan una garantía casi perfecta de
               que los contenidos de su servidor réplica son idénticos o los del servidor
               central de Debian. Aun así, no son perfectos. Teóricamente, hay varias maneras
               en las que un paquete modificado se pueda introducir en el archivo central de
               Debian.

               Asegurando que solo pueda instalar desde una fuente firmada le dará un alto
               grado de protección frente a los paquetes maliciosos, pero no puede eliminar
...
que los ficheros de paquete listados en el índice son los mismos que el
...
llave pública del archivo de Debian se distribuye con apt, generalmente a través
de su CD de Debian.

Cuando aptitude descarga el índice de un archivo, comprobará si el índice está
apropiadamente firmado. Si no está firmado aptitude no confiara en los paquetes
provenientes de ese archivo (más adelante se explicará lo que esto significa).
Si tiene una firma pero es incorrecta o no se puede verificar, verá un aviso y
aptitude se negará a confiar en paquetes procedentes de ese archivo.
...
Puede encontrar útil que apt confié en archivos externos aparte de los ficheros
de Debian. Para cada paquete en el que quiera confiar, tendrá que adquirir la
llave pública que se utiliza para firmar el índice del archivo de paquetes. Éste
es por lo general un fichero de texto cuyo nombre finaliza en .asc; puede ser
suministrado por el administrador del sitio web o descargado desde un servidor
de llaves públicas. Para más información acerca de llaves públicas y de como
conseguirlas, consulte la página web de GPG.

La lista de llaves en las que apt confía se guarda en el fichero del anillo de
...
IMG.alt.suffix Aviso
 = \[Aviso]]
               Una vez que se añade un archivo de una llave al anillo de llaves de APT, !se le
               otorgara la misma confianza que a los propios servidores réplica de Debian¡ Haga
               esto sólo si está muy seguro de que la llave que está añadiendo es la correcta y
               si la persona que posee la llave es competente y de toda confianza.
...
clanbomber, debe también instalar el paquete libclanlib2. Si elimina clanbomber,
probablemente no necesite más libclanlib2; aptitude intentará detectar esto y
...
paquetes se marcan como “automáticamente instalados”; aptitude los registrará y
eliminará cuando ya no sean dependencia de cualquier paquete manualmente
instalado [10] . Aparecerán en la previsualización como “paquetes que se
eliminarán porque ya no se usan.”
...
“automático” en cualquier momento pulsando m; si ya se eliminó el paquete, puede
usar Paquete → Instalar (+) para cancelar la eliminación y borrar la marca de
“automático”.
...
El primero es un algoritmo que se emplea también en programas tales como apt-get
y synaptic; Me referiré a el como “resolución inmediata”. Se invoca cuando marca
un paquete para su instalación de forma interactiva, e inmediatamente después de
que uno o más paquetes son marcados en la línea de órdenes. La resolución
inmediata es rápida y solucionará la mayoría de problemas de dependencias,
aunque a veces no pueda encontrar ninguna.

El segundo algoritmo, que llamaré “resolución interactiva”, se invoca cuando hay
paquetes con dependencias rotas incluso después de la resolución inmediata[11].
Puede resolver más dependencias, le permite previsualizar la solución antes de
aplicarla, y le permitiría también introducir respuestas al solucionador, para
así guiarle a una solución más adecuada.
...
dependencia insatisfecha (sea un “Depende”, “Recomienda”, o con un “Conflicto”),
realizará los siguientes pasos:


  1. Si la dependencia es una recomendación, aptitude intentará ver si es una
     recomendación “nueva” o una “recomendación” ya “satisfecha”. aptitude
     considera una recomendación como “nueva” si el paquete manifestando la
     recomendación no está instalado, o si la versión instalada no recomienda un
     paquete del mismo nombre. Por otro lado, una recomendación está
     “satisfecha” si el paquete ya instalado recomienda un paquete del mismo
     nombre, estando ya satisfecho.
     ...
     “nueva” porque la versión 1.0 de prog no recomendaba apache. Por otro lado,
     la recomendación de libcool1 no es “nueva”, porque la versión1.0 de prog
     recomienda libcool1, aunque recomiende una versión diferente. De todas
     formas, si libcool1 está instalado, entonces la recomendación se tratará
     como “ya satisfecha”.
     ...
     todas las demás se ignorarán en la siguiente resolución. Si la opción es
     false, la resolución inmediata de dependencias ignorará todas las
     recomendaciones.

  2. Si la dependencia se da en varios paquetes en combinación con «OR», examine
     cada una de las alternativas en el orden dado. Por ejemplo, si un paquete
     depende de “exim | mail-transport-agent”, aptitude procesará primero exim,
     y después mail-transport-agent.

  3. Intente resolver cada alternativa dada. Si la dependencia es un conflicto,
     elimine la alternativa actual si ya está instalada (y por cada conflicto no
     de versiones, elimine también cualquier paquete que provoca el núcleo del
     conflicto). También puede instalar la versión candidata de la alternativa
     actual si satisface la dependencia. Si no, o si no hay otra versión
     candidata (p. ej., porque la alternativa actual es un paquete virtual), y
     si la dependencia no tiene versión, intente instalar el paquete de más alta
     prioridad[12] cuya versión candidata provea el objetivo de la alternativa
     actual.

     Por ejemplo, imagine que estamos intentando resolver “Depende: exim | mail-
     transport-agent”. En primer lugar, aptitude intentará instalar el paquete
     exim. Si exim no está disponible, aptitude intentará entonces instalar el
     paquete con la prioridad más alta cuya versión candidata provee exim. Si no
     encuentra tal paquete, aptitude instalará el paquete con la prioridad más
     alta cuya versión candidata provee el paquete virtual mail-transport-agent.
     Por otro lado, imagine que la dependencia es “Depende: exim (>= 2.0.0) |
     mail-transport-agent”, pero cuya única versión de exim disponible es 1.0.
     En este caso, aptitude no instalará exim (porque la versión no corresponde
     a la dependencia), ni intentará instalar paquetes que provean exim (porque
     ...
     paquete con la prioridad más alta cuya versión candidata provea mail-
     transport-agent.
...
* La manera de resolver un conflicto es desinstalando el paquete que es el
  ...
  dependencias no resueltas; el solucionador inmediato no realiza ninguna acción
  para arreglarlo.
...
* Puede que nunca se satisfaga una dependencia por razones de restricciones de
  ...
  declara una dependencia “Depende: fileutils (>= 2.0)”. El solucionador
  inmediato es incapaz de resolver esta dependencia pues nunca considerará la
  versión 2.0 del mismo paquete puesto que no es la versión candidata.
...
pulsa . y ,, aplicar la solución en pantalla pulsando !, y examinar la solución
más detenidamente pulsando e. Ud. vería una pantalla similar a la siguiente si
examinase este mismo problema.
...
aprueba una acción, el solucionador la tomará siempre que sea posible, ignorando
otras alternativas (cuando hay más de una acción aprobada entre las
...
deshacer esta acción empleando Deshacer → Deshacer (Control+u) a la vez que la
pantalla del solucionador está activa. Si cancela un rechazo o una
desinstalación, cualquier solución que se ignoró estará disponible la siguiente
...
IMG.alt.suffix Nota
  = \[Nota]]
               ...
               paquetes retenidos, o que instalan versiones prohibidas de ciertos paquetes.
               Puede invalidar estos rechazos, y por ello anular el estado configurado, de la
               misma forma que puede invalidar cualquier otro rechazo. Si configura la opción
               Aptitude::ProblemResolver::Allow-Break-Holds como true desactivaría estas
               acciones, lo cual quiere decir que el solucionador siempre romperá retenciones
               (aunque con una penalización, consulte Aptitude::ProblemResolver::
               BreakHoldScore).


Las acciones rechazadas se muestran de color rojo y marcadas con una “R”,
mientras que las acciones aprobadas son verdes y se marcan con una “A”. Puede
ver esto en la siguiente imagen, donde la acción “mantener gstreamer0.8-
cdparanoia en su versión presente” se ha rechazado, y que la acción “mantener
sound-juicer en su versión presente” se aprobó.
...
cuando el numero que se encuentra frente al paréntesis y el que está dentro son
idénticos (como aparece arriba), pulsar «.» genera una nueva solución. Si no hay
ningún número entre paréntesis, (por ejemplo, si el indicador muestra [1/5]),
entonces no quedan más soluciones por generar. En cualquier momento, puede
seleccionar la última solución generada pulsando >, o < para ver la primera
solución generada.


IMG.alt.suffix Importante
     = \
[Importante]]  El estado del solucionador de problemas cambia cuando modifica el estado de
               cualquier paquete. Si marca un paquete para instalar, actualizar o eliminar,
               etc... el solucionador desecha todos los rechazos y aprobaciones, así como las
               soluciones que haya generado hasta el momento.


Además de las acciones que puede seleccionar de la lista en la parte superior de
la pantalla, también puede seleccionarlas usando la lista en la parte inferior
de la pantalla. Para acceder a ella use el ratón o pulse Tab. Por último, para
ver las decisiones que el solucionador tomó por orden, pulse o. Esto dará un
lista de las dependencias que se resolvieron y la acción tomada para ello, como
...
dependencias es un valor que aptitude utiliza para determinar cuán “mala” es una
solución. Las “mejores” soluciones siempre aparecen antes de las “peores”
soluciones. El coste de las soluciones se definen con la opción de configuración
...
El coste predeterminado, que orden las soluciones por su coste de seguridad, y
después por su prioridad de anclaje de apt:

safety, priority

Elimina el menor número de paquetes posible, después cancela el menor número de
acciones:

eliminaciones, acciones-canceladas

Ordena soluciones por el número de paquetes que eliminan además del doble del
número de las acciones que cancela.

removals + 2 * canceled-actions



Como se puede ver en los ejemplos anteriores, el coste no es necesariamente un
único número. De hecho, un coste consiste de uno más componentes del coste,
siendo cada uno un número asociado a una solución. Al ordenar las soluciones, el
solucionador examina los componentes del coste en orden, procediendo a los
siguientes componentes una vez que los anteriores son iguales. Por ejemplo, en
el coste “removals, canceled-actions”, los soluciones que eliminan menos
paquetes siempre aparecen antes que las soluciones que eliminan más, sin
importar el número de acciones canceladas que conllevan. Por otra parte, las
soluciones con el mismo número de eliminaciones se ordenan de manera que las
acciones con menos acciones canceladas aparecen primero.

Los componentes del coste son de dos tipos: componentes de coste básicos y
componentes de coste compuestos.

Los componentes básicos simplemente nombran una propiedad de la solución, como
“upgrades” o “removals”. Puede ver una lista de los componentes básicos
integrados que aptitude ofrece en Tabla 2.1, “Componentes de coste básicos”.
También puede crear sus propios componentes de coste usando las indicaciones
add-to-cost-component y raise-cost-component; para más detalles consulte
...
eliminar paquetes o instalar paquetes nuevos), mientras que los niveles asocian
un número con cada acción para computar el número más alto asociado con
cualquier acción en la solución.

Tabla 2.1. Componentes de coste básicos

 _________________________________________________________________________________________________
|Nombre   |Tipo    |Descripción                                                                  |
|_________|________|______________________________________________________________________________|
|broken-  |Contador|Cuenta el número de retenciones que la solución rompe, si es que se permite |
|holds    |        |que el solucionador los rompa (Aptitude::ProblemResolver::Allow-Break-Holds). |
|_________|________|______________________________________________________________________________|
|canceled-|Contador|Cuenta el número de acciones pendientes que la solución cancela (esto es,   |
|actions  |        |mantiene los paquetes en su versión presente).                               |
|_________|________|______________________________________________________________________________|
|installs |Contador|Cuenta el número de paquetes que la solución instala.                       |
|_________|________|______________________________________________________________________________|
|non-     |        |Cuenta el número de versiones que la instalación instala o actualiza desde  |
|default- |Contador|fuentes no predeterminadas.                                                   |
|versions |        |                                                                              |
|_________|________|______________________________________________________________________________|
|         |        |Un valor que aumenta a medida que disminuye la prioridad de anclaje de apt.   |
|priority |Nivel   |Específicamente, se computa haciendo negativa la prioridad de anclaje (por   |
|         |        |ejemplo, si la prioridad de anclaje es 500, este componente computará como - |
|         |        |500).                                                                         |
|_________|________|______________________________________________________________________________|
|removals |Contador|Cuenta el número de paquetes que la solución elimina.                       |
|_________|________|______________________________________________________________________________|
|removals-|Contador|Cuenta el número de paquetes manualmente instalados que la solución elimina.|
|of-manual|        |                                                                              |
|_________|________|______________________________________________________________________________|
|safety   |Nivel   |Una amplia heurística que aumenta a medida que las acciones son menos        |
|         |        |“seguras”; para más detalles consulte “Costes de seguridad”.         |
|_________|________|______________________________________________________________________________|
...
básicos. Por ejemplo, removals + canceled-actions (eliminaciones + acciones
canceladas) suma los componentes removal y canceled-actions, resultando en un
componente que enumera el número de eliminaciones y de acciones canceladas. Los
componentes compuestos combinan contadores mediante la suma, y escoge el nivel
tomando el valor máximo como se ve en Figura 2.11, “Sintaxis de componentes de
coste compuestos”.
...
IMG.alt.suffix Nota
  = \[Nota]]
               ...
               niveles y contadores de cualquier manera. Por ejemplo, los costes removals +
               safety y max(upgrades, installs) se tomarán como errores, y el solucionador los
               ignorará.[13]
...
Tenga en cuenta que cada componente básico individual se puede multiplicar por
un factor de escala antes de su combinación con otros componentes. Se puede usar
para controlar las negociaciones que el solucionador realiza entre costes. Por
ejemplo, un coste de 2*removals + 3*upgrades dice que tres eliminaciones son
igualmente “malas” que dos actualizaciones. Las soluciones que contienen cuatro
eliminaciones y una actualización se considerarán equivalentes a soluciones que
contienen una eliminación y tres actualizaciones, ya que ambos tiene un coste de
once.
...
IMG.alt.suffix = \Niveles de coste de seguridad]

[Safety cost levels diagram]



El componente de coste safety (seguridad) es una estimación heurística de lo
“segura” o “insegura” que es una solución. Los costes de seguridad se pueden
imaginar como una forma de dividir soluciones en varios “niveles”, donde los
niveles “menos seguros” reciben número más altos. Figura 2.12, “Niveles de coste
de seguridad” muestra cómo esto funciona con la configuración predeterminada de
aptitude.


IMG.alt.suffix Sugerencia
     = \
[Sugerencia]]  Los niveles de coste de seguridad son sólo una forma de controlar el orden en que
               aparecen las soluciones de dependencia. Consulte “Costes en el solucionador
               interactivo de dependencias.” para una descripción completa de cómo modificar el
               orden en que aptitude ordena las soluciones.


Por omisión aptitude inicia el solucionador con un conjunto “apreciable” de
niveles de coste de seguridad. Éstos son:
...
|coste |                                                                              |                         |
|______|______________________________________________________________________________|_________________________|
|      |                                                                              |Aptitude::               |
|      |Las soluciones que sólo incluyen acciones “seguras” (instalar la versión|ProblemResolver::Safe-   |
|10,000|predeterminada de un paquete, o mantener un paquete en su versión actual) y l|Level, Aptitude::        |
|      |eliminación de paquetes.                                                     |ProblemResolver::Remove- |
|      |                                                                              |Level                    |
|______|______________________________________________________________________________|_________________________|
|      |                                                                              |Aptitude::               |
|20,000|La solución que cancela todas las acciones del usuario.                      |ProblemResolver::Keep-   |
|      |                                                                              |All-Level                |
|______|______________________________________________________________________________|_________________________|
|      |Soluciones que rompen retenciones definidas por el usuario o que instalan     |Aptitude::               |
|40,000|versiones prohibidas.                                                         |ProblemResolver::Break-  |
|      |                                                                              |Hold-Level               |
|______|______________________________________________________________________________|_________________________|
|      |Soluciones que instalan paquetes con versiones no predeterminadas (tales como |Aptitude::               |
|50,000|“experimental”, por ejemplo).                                             |ProblemResolver::Non-    |
|      |                                                                              |Default-Level            |
|______|______________________________________________________________________________|_________________________|
|      |                                                                              |Aptitude::               |
|60,000|Soluciones que desinstalan paquetes Esenciales.                               |ProblemResolver::Remove- |
|      |                                                                              |Essential-Level          |
|______|______________________________________________________________________________|_________________________|
...
paquete correspondería al nivel 40,000. Puede ajustar los niveles de versiones
de manera individual usando las indicaciones del solucionador; para más
detalles, consulte “Configurar indicaciones del solucionador”. Los niveles
predeterminados se ilustran en Figura 2.12, “Niveles de coste de seguridad”.
...
fuertemente hacia otra versión o paquete, o se pueden usar para “pre-cargar” el
solucionador con rechazos y aprobaciones, al igual que si hubiese entrado en el
solucionador y rechazado o aprobado varias versiones manualmente.

Las indicaciones se guardan en el fichero de configuración de apt, /etc/apt/
apt.conf, dentro del grupo de configuración.“Aptitude::ProblemResolver::Hints”
(consulte “Referencia del fichero de configuración.” para más detalles acerca
...
ubica uno o más paquetes usando el valor del objetivo, elije una o más versiones
de esos paquetes usando el valor de la versión y, por último, ejecuta la acción.
...
ser uno de los siguientes:
  ...
  2. “reject”: Rechaza la versión, al igual que si ejecuta la orden Solucionador
     → Conmutar Rechazados (r).

  3.  “discard”: Descarta cada solución que contenga la versión. Se diferencia
     con “reject” en que no es visible, de forma que el usuario no lo puede
  ...
  4. “increase-safety-cost-to número”: Aumenta el coste de seguridad de
     ...
     seguridad ya es superior al número, esta indicación no tendrá efecto. El
     ...
     consulte “Costes y componentes del coste” y “Costes de seguridad”.

     Se pueden escoger algunos niveles de coste especiales por nombre:


       a. conflict, discard: En lugar de cambiar el coste de seguridad, descarta
          soluciones que contienen la versión como si se hubiese aplicado la
          indicación “discard”.

       b. maximum: El coste máximo de seguridad.

       c. minimum: El coste de seguridad más bajo. Todas las búsquedas comienzan
          con este coste, así que “aumentar” una versión a este coste no tiene
          ningún efecto. Por otro lado, también puede emplear este valor para
          ajustar los niveles de coste predeterminados: por ejemplo, definir
          Aptitude::ProblemResolver::Remove-Level como “minimum” hace que los
          paquetes desinstalados no tengan ningún efecto en el coste de
     ...
     IMG.alt.suffix Nota
       = \[Nota]]
                    La indicación increase-safety-cost-to se aplica en adición a cualquier
                    coste de seguridad predeterminado que se corresponda con la acción
                    seleccionada. Por ejemplo, una indicación que aumenta el coste de seguridad
                    de “instalar hal desde experimental” a 15.000 no tiene efecto alguno,
                    porque la acción ya tiene un coste de seguridad de 15.000 (suponiendo que
                    esta versión de hal no es la versión candidata predeterminada).


  5. “número”: añadir el número a la puntuación de la versión, inclinando al
     solucionador a favor del mismo o (con un número negativo) en contra. Por
     ejemplo, la indicación 200 emacs añade 200 a la puntuación de emacs,
     mientras que la indicación -10 emacs sustrae 10 a su puntuación.


Si el campo de objetivo de una indicación del solucionador contiene un signo de
interrogación (“?”) o tilde (“~”), se toma como un patrón de búsqueda y se
consideran todas las versiones de paquetes que encajen . De otra manera, se toma
como el nombre de un paquete a seleccionar. Así que el objetivo “g++” sólo
seleccionaría el paquete g++, pero el objetivo “?section(non-free)”
seleccionaría cualquier paquete dentro de la sección non-free. Para más
...
verán afectadas por la indicación. De no ser así, puede tener cualquiera de las
siguientes formas:
  ...
  8. “:UNINST”: en lugar de afectar a cualquier otra versión del objetivo, la
     indicación afecta a la decisión de desinstalar el objetivo. Por ejemplo,
     “reject aptitude :UNINST” impide al solucionador intentar desinstalar
     aptitude.

  9. “versión”: la indicación afecta solo a las versiones cuyo número de versión
     es versión.
...
lista en aptitude se conoce como “patrón de búsqueda”. Mientras que el uso más
básico de los patrones de búsqueda es el de emparejar por nombre de paquete,
aptitude le permite realizar búsquedas mucho más complejas. Además de la
interfaz gráfica, algunas operaciones en la línea de órdenes pueden emplear
patrones de búsqueda; consulte Referencia de la línea de órdenes para más
detalles.

Un patrón de búsqueda consiste de una o más condiciones (también conocidas como
“términos”); los paquetes se corresponden con el patrón si se corresponden con
todos sus términos. Por lo general, los términos empiezan con un signo de
interrogación (“?”), seguido del nombre del término, el cual describe la
búsqueda que el término realiza: por ejemplo, el término ?name se corresponde
con los nombres de los paquetes, mientras que el término ?version busca entre
las versiones de los paquetes. Por último, cualquier parámetro adicional al
término de búsqueda se escribe entre paréntesis, (consulte la documentación de
términos individuales para ver los detalles de lo que significa cada parámetro
de los diferentes términos).


IMG.alt.suffix Nota
  = \[Nota]]
               Un texto que no venga precedido de “?” forma también un patrón de búsqueda:
               aptitude trata cada palabra (o cadena entrecomillada) como un argumento para el
               patrón ?name, el cual busca un paquete cuyo nombre coincide con el texto cuando
               éste se interpreta como una expresión regular.



IMG.alt.suffix Aviso
 = \[Aviso]]
               El comportamiento de aptitude cuando se introduce un patrón de búsqueda sin “?” (o
               “~”) está así dispuesto para facilitar el uso interactivo, y cambiará en el futuro;
               aquellos scripts que invoquen aptitude deberán nombrar la estrategia de búsqueda de
               manera explicita. Ésto es, que los scripts deberían buscar “?name(coq)” en lugar de
               “coq”).
...
introducir cadenas entrecomilladas (“"”), con lo cual “?name(scorch)” y “?name
("scorch")” realizarían la misma búsqueda. Si introduce una cadena de búsqueda
empleando comillas dobles puede también incluir comillas dobles literales
poniendo un barra invertida (“\”) justo delante: por ejemplo, “?description
("\"easy\"")” coincidiría con todo paquete cuya descripción contenga la cadena
en cuestión.
...
búsqueda.[14] Para que estos caracteres pierdan su significado especial, ponga
una tilde (“~”) delante de ellos. Por ejemplo, “?description(etc))” contiene un
error de sintaxis, porque el primer “)” cierra el término ?description, y el
segundo “)” no se corresponde con ningún “(”. En contraste, “?description
(etc~))” sí muestra cualquier paquete cuyo texto contiene “etc)”. Hay también
consideraciones adicionales si está empleando la abreviatura de un término;
consulte “Abreviaturas de términos de búsqueda.” para más detalles.

La mayoría de búsquedas de texto (nombres de paquete, descripciones, etc...) se
realizan usandoexpresiones regulares no sensibles a las mayúsculas. Una
expresión regular coincide con un campo si cualquier porción del campo de texto
coincide con la expresión; por ejemplo, “ogg[0-9]” coincide con “libogg5”,
“ogg123”, y “theogg4u”. Algunos caracteres tienen un significado especial dentro
de una expresión regular [15] , así que si desea encontrarlos en las búsquedas
tendrá que introducir un escape de barra inversa: por ejemplo, para encontrar
“g++” debería usar el patrón “g\+\+”.

Los caracteres “!” y “|” tienen un significado especial dentro de un patrón de
búsqueda. Para poder incluir estos caracteres en una cadena no entrecomillada,
puede precederlos de una tilde (“~”). Por ejemplo, para encontrar paquetes cuya
descripción contiene “grand” u “oblique”, use el patrón “?description
(grand~|oblique)”. De todas formas, en estos casos posiblemente encuentre más
sencillo usar una cadena entrecomillada: “?description("grand|oblique")”.


Abreviaturas de términos de búsqueda.

Algunos términos de búsqueda se pueden escribir usando formas “cortas”, las
cuales consisten de una tilde (“~”) seguida de un único carácter que identifica
el término y por último, los argumentos (en caso de haberlos) para el término.
Por ejemplo, la forma abreviada de ?name(aptitude) es ~n aptitude.

Si escribe un término empleando su forma abreviada, los caracteres de tilde y
“espacios en blanco”, esto es, caracteres de espacio, tabulados y similares, se
dividirá el término y se iniciará un nuevo término. Por ejemplo, “~mDaniel
Burrows” buscaría cualquier paquete cuyo campo de desarrollador contenga
“Daniel” y cuyo nombre contenga “Burrows”, mientras que “~i~napt” busca paquetes
instalados cuyo nombre contiene apt. Para incluir espacios en blanco en la
expresión de búsqueda, puede poner una tilde delante del mismo (como en Daniel~
Burrows) o introducir unas comillas (como en "Debian Project", o incluso Debian"
"Project). Dentro de una cadena entrecomillada, puede emplear la barra invertida
(“\”) para cancelar el significado especial de unas comillas: por ejemplo,
...
IMG.alt.suffix Nota
  = \[Nota]]
               Los signos de interrogación (“?”) no pueden cerrar la forma abreviada de un término,
               aunque vayan seguidos por el nombre de un término de búsqueda. Por ejemplo,
               “~napt?priority(required)” busca cualquier paquete cuyo nombre se corresponde con la
               expresión regular “apt?priority(required)”. Añada uno o más espacios entre los
               términos si desea combinar un término corto de consulta con un término de búsqueda
               definido con el nombre, como por ejemplo “~napt ?priority(required)”, o introduzca
               signos de interrogación en torno al texto (de haberlo) siguiendo la forma abreviada
               del término, como puede ver en “~n"apt"?priority(required)”.


Tabla 2.3, “Guía rápida de términos de búsqueda” presenta en una lista la forma
abreviada de cada término de búsqueda.


Búsquedas y versiones.

De manera predeterminada, un patrón se corresponde con un paquete si cualquier
versión del paquete coincide con el patrón. De todas formas, algunos patrones
...
cualquier paquete que dependa de un paquete que coincide con patrón. De todas
formas, patrón solo coincide con las versiones de un paquete que satisfacen la
...
algunos patrones coincidirían con una versión pero no con la otra. Por ejemplo,
el patrón ?installed solo muestra la versión del paquete que está instalado. De
manera similar, el patrón ?maintainer(desarrollador) solo encontraría versiones
que contengan el desarrollador dado. Normalmente, todas las versiones de un
paquete tienen el mismo desarrollador, pero éste no es siempre el caso; de
hecho, cualquier patrón de búsqueda que examina los campos de un paquete (aparte
de su nombre, por supuesto) actúa de esta manera, pues todos los campos de un
paquete pueden variar de una versión a otra.
...
depende de un paquete B, donde B coincide con la versión 2.0, independientemente
de si la versión satisface la dependencia de A. Por otro lado, el término
?narrow restringe las versiones con las cuales se compara el sub-patrón: ?narrow
(?installed, ?depends(?version(ubuntu))) encontraría cualquier paquete cuya
versión instalada tiene una dependencia que se puede satisfacer con un paquete
cuya cadena de versión contenga “ubuntu”.


IMG.alt.suffix Nota
  = \[Nota]]
               Hay una sutil pero importante distinción entre emparejar un patrón con un paquete,
               y emparejarlo con todas las versiones de ese paquete. Cuando un patrón se compara
               con un paquete cada uno de sus términos se compara con el paquete, y por lo tanto
               cada término coincidiría si cualquier versión del paquete coincide. En contraste,
               cuando un patrón se compara con cada versión de un paquete, coincidiría con éxito
               al emparejarse cuando todos sus términos coinciden con la misma versión del
               paquete.

               Por ejemplo: suponga que la versión 3.0-1 del paquete aardvark está instalado,
               pero está disponible la versión 4.0-1. La expresión de búsqueda ?version(4\.0-
               1)?installed muestra aardvark, porque ?version(4\.0-1) coincide con la versión
               4.0-1 de aardvark, mientras que ?installed coincide con la versión 3.0-1. Por otra
               parte, esta expresión no se emparejaría con todas las versiones de aardvark,
               porque no hay ninguna versión instalada y porque también tiene el numero de
               versión 4.0-1.
...
usando objetivos explícitos. En una expresión normal de búsqueda no hay ninguna
manera de referirse al paquete o a la versión que se revisa en el momento. Por
ejemplo, suponga que desea realizar una búsqueda de todo paquete P que depende
de un segundo paquete Q de manera que Q recomienda P. Obviamente, necesita
comenzar con el término ?depends(...). Pero el término introducido en ...
necesita seleccionar paquetes idénticos a aquel emparejado con ?depends. Cuando
he descrito esta meta traté con este tema dando los nombres de los paquetes,
denominándolos P y Q; términos con objetivos explícitos harían exactamente lo
mismo. [17]
...
Funciona de la misma manera que patrón, pero puede emplear la variable dentro de
patrón para referirse al paquete o versión con el cual se empareja patrón. Puede
usar variable de dos maneras:
  ...
  1. El término ?= coincidiría de manera exacta con el paquete o la versión
     ...
     correspondiente se limita a una sola versión, ?= coincidiría o bien con la
     versión (si ?= se ha limitado) o con el paquete completo; de otra forma,
     coincide con cualquier versión del paquete.

     Véase Ejemplo 2.2, “ Uso del término ?=. ” para ver un ejemplo de uso de
     ?=.

  2. El término ?bind(variable, patrón) coincide con cualquier paquete o versión
     si el valor de variable coincide con patrón.

     Hay una forma abreviada para términos de tipo ?. La expresión ?bind
     (variable, ?término[(argumentos)]) se puede sustituir por ?variable:término
     (argumentos).
...
Tabla 2.3, “Guía rápida de términos de búsqueda” proporciona un escueto resumen
de todos los términos de búsqueda en aptitude. Puede consultar la descripción
completa de cada término a continuación.

Tabla 2.3. Guía rápida de términos de búsqueda

 ___________________________________________________________________________________________________________________________________________________________
|Forma larga                     |Forma abreviada                            |Descripción                                                                  |
|________________________________|___________________________________________|______________________________________________________________________________|
|?=variable                      |                                          |Selecciona el paquete ligado a variable; consulte “Objetivos explícitos de |
|                                |                                           |búsqueda.”.                                                                |
|________________________________|___________________________________________|______________________________________________________________________________|
|?not(patrón)                   |!patrón                                   |Selecciona el paquete que no coincide con patrón.                            |
|________________________________|___________________________________________|______________________________________________________________________________|
|?action(acción)                |~aacción                                  |Selecciona paquetes que se han marcado para la acción dada (p. ej., “instal|”
|                                |                                           |o “upgrade”).                                                             |
|________________________________|___________________________________________|______________________________________________________________________________|
...
|________________________________|___________________________________________|______________________________________________________________________________|
|?architecture(arquitectura)     |~rarquitectura                             |Selecciona paquetes para la arquitectura dada (por ejemplo, “amd64” o “a|l”).
|                                |                                           |Valores especiales: native y foreign.                                         |
|________________________________|___________________________________________|______________________________________________________________________________|
...
|________________________________|___________________________________________|______________________________________________________________________________|
|?bind(variable, patrón)        |?variable:nombre_del_término[(argumentos)]|Selecciona cualquier variable que coincide con patrón; consulte “Objetivos |
|                                |                                           |explícitos de búsqueda.”.                                                 |
|________________________________|___________________________________________|______________________________________________________________________________|
...
|________________________________|___________________________________________|______________________________________________________________________________|
|?broken-tipodep(patrón)        |~DB[tipodep:]patrón                       |Selecciona cualquier paquete con una dependencia rota del tipodep dado que    |
|                                |                                           |encaje con patrón.                                                           |
|________________________________|___________________________________________|______________________________________________________________________________|
|?broken-reverse-tipodep(patrón)|~RB[tipodep:]patrón                       |Selecciona paquetes sobre los que un paquete que coincide con él patrón     |
|                                |                                           |declara una dependencia rota del tipo tipodep.                                |
|________________________________|___________________________________________|______________________________________________________________________________|
|?conflicts(patrón)             |~Cpatrón                                  |Selecciona paquetes que entran en conflicto con un paquete que coincide con   |
|                                |                                           |patrón.                                                                      |
|________________________________|___________________________________________|______________________________________________________________________________|
|?config-files                   |~c                                         |Selecciona paquetes desinstalados pero no purgados.                           |
|________________________________|___________________________________________|______________________________________________________________________________|
|?tipodep(patrón)               |~D[tipodep:]patrón                        |Selecciona paquetes que declaran una dependencia de tipo tipodep sobre un     |
...
|________________________________|___________________________________________|______________________________________________________________________________|
|?essential                      |~E                                         |Selecciona paquetes esenciales, aquellos con Essential: yes en sus ficheros de|
|                                |                                           |control.                                                                      |
|________________________________|___________________________________________|______________________________________________________________________________|
...
|________________________________|___________________________________________|______________________________________________________________________________|
|?for variable: patrón          |                                          |Selecciona paquetes que coinciden el patrón con la variable ligada al paquete|
|                                |                                           |con que se empareja; consulte “Objetivos explícitos de búsqueda.”.      |
|________________________________|___________________________________________|______________________________________________________________________________|
|?garbage                        |~g                                         |Selecciona paquetes que ningún paquete instalado manualmente requiere.       |
|________________________________|___________________________________________|______________________________________________________________________________|
|?installed                      |~i                                         |Selecciona paquetes instalados.                                               |
|________________________________|___________________________________________|______________________________________________________________________________|
|?maintainer(desarrollador)      |~mdesarrollador                            |Selecciona paquetes cuyo responsable es el desarrollador.                     |
|________________________________|___________________________________________|______________________________________________________________________________|
|?multiarch(multiarquitectura)   |                                          |Selecciona paquetes con la funcionalidad multiarquitectura definida (esto es, |
|                                |                                           |“foreign”, “same”, “allowed”, o “none”).                      |
|________________________________|___________________________________________|______________________________________________________________________________|
|?narrow(filtro, patrón)        |~S filtro patrón                          |Selecciona paquetes que coinciden con ambos filtro y patrón en una sola      |
|                                |                                           |versión.                                                                     |
|________________________________|___________________________________________|______________________________________________________________________________|
...
|________________________________|___________________________________________|______________________________________________________________________________|
|?reverse-tipodep(patrón)       |~R[tipodep:]patrón                        |Selecciona paquetes que son objetivo de una dependencia de tipo tipodep       |
|                                |                                           |declarado por un paquete que coincide con patrón.                            |
|________________________________|___________________________________________|______________________________________________________________________________|
|?reverse-broken-tipodep(patrón)|~RB[tipodep:]patrón                       |Selecciona paquetes que son el objetivo de una dependencia rota de tipo       |
|                                |                                           |tipodep declarado por un paquete que coincide con patrón.                    |
|________________________________|___________________________________________|______________________________________________________________________________|
|?section(sección)              |~ssección                                 |Selecciona paquetes en la sección dada.                                      |
|________________________________|___________________________________________|______________________________________________________________________________|
|?source-package(nombre)         |                                          |Selecciona paquetes cuyo nombre de paquete fuente coincide con la expresión  |
|                                |                                           |regular nombre.                                                               |
|________________________________|___________________________________________|______________________________________________________________________________|
|?source-version(versión)       |                                          |Selecciona paquetes cuya versión de paquete fuente coincide con la expresión|
|                                |                                           |regular versión.                                                             |
|________________________________|___________________________________________|______________________________________________________________________________|
...
|________________________________|___________________________________________|______________________________________________________________________________|
|?term-prefix(palabra_clave)     |                                          |Búsqueda completa de texto de paquetes que contienen una palabra clave que   |
...
|________________________________|___________________________________________|______________________________________________________________________________|
|?user-tag                       |                                          |Selecciona paquetes marcados con una etiqueta de usuario que encaje con la    |
|                                |                                           |expresión regular user-tag (etiqueta de usuario).                            |
|________________________________|___________________________________________|______________________________________________________________________________|
|?version(versión)              |~Vversión                                 |Selecciona paquetes cuya versión coincide con versión (valores especiales:  |
|                                |                                           |CURRENT, CANDIDATE, y TARGET).                                                |
|________________________________|___________________________________________|______________________________________________________________________________|
|?virtual                        |~v                                         |Selecciona paquetes virtuales.                                                |
|________________________________|___________________________________________|______________________________________________________________________________|
|                                |                                           |Selecciona versiones para los cuales el patrón coincide con cualquier versió|
|?widen(patrón)                 |~Wpatrón                                  |del paquete correspondiente, descartando las restricciones locales de         |
|                                |                                           |versiones.                                                                    |
|________________________________|___________________________________________|______________________________________________________________________________|
  ...
  nombre
      ...
      IMG.alt.suffix Nota
        = \[Nota]]
                     ...
                     nombres contengan diferentes sub-cadenas; por ejemplo, “?name(apti)?name
                     (tude)” busca cualquier paquete cuyo nombre contiene ambos “apti” y
                     “tude”.
  ...
  ?not(patrón), !patrón
      Buscar paquetes que no coinciden con el patrón patrón. Por ejemplo, “?not
      (?broken)” selecciona paquetes que no están “rotos”.


      IMG.alt.suffix Nota
        = \[Nota]]
                     Para poder incluir “!” en una cadena de búsqueda, debe tener un “escape”
                     introduciendo una tilde (“~”) delante de el; de no ser así, aptitude lo
                     considerará como parte de un término ?not. Por ejemplo, para seleccionar paquetes
                     cuya descripción contenga “extra!”, use “?description(extra~!)”.
  ...
  ?or(patrón1, patrón2), patrón1 | patrón2
      ...
      IMG.alt.suffix Nota
        = \[Nota]]
                     Para poder emplear el carácter “|” en una expresión regular, debe tener un
                     “escape” para impedir que aptitude genere un término OR de el: “~|”.
  ...
  ?any-version(patrón)
      ...
      IMG.alt.suffix Nota
        = \[Nota]]
                     Este término tiene una estrecha relación con ?narrow. De hecho, ?any-
                     version(patrón1 patrón2) es exactamente lo mismo que ?narrow(patrón1,
                     patrón2).



  ?architecture(arquitectura), ~rarquitectura
      Compara versiones de paquetes para la arquitectura dada. Por ejemplo,
      “?architecture(amd64)” coincide con paquetes amd64, mientras que
      “?architecture(all)” coincide con todos los paquetes independientes de
      arquitectura. También acepta los valores especiales native y foreign.
  ...
  ?for variable: patrón
      ...
      (apt). Además, el término ?=variable selecciona todo paquete o versión que
      se corresponde con el valor de la variable.
  ...
  ?priority(prioridad), ~pprioridad
      ...
      (required)” muestra aquellos paquetes con una prioridad “required”.
  ...
  ?reverse-broken-tipodep(patrón), ?broken-reverse-tipodep(patrón), ~RB[tipodep:
  ]patrón
      tipodep puede ser “provides” o uno de los tipos de dependencias ilustrados
      en la documentación de ?broken-tipodep. Si tipodep no está presente,
      depends es el argumento predeterminado.
  ...
  ?tag(etiqueta), ~Getiqueta
      ...
      Para más información acerca de etiquetas y debtags, consulte http://
   ...
   ?version(versión), ~Vversión
      ...
      * CANDIDATE busca la versión, de existir, del paquete que se instalaría si
        pulsa + sobre el paquete o ejecuta aptitude install sobre el paquete.
...
Esta sección describe como configurar los contenidos y el formato de la lista de
paquetes, la línea de estado y la de cabecera, así como la salida de aptitude
search.

Puede definir el formato de cada uno de estos espacios con una “cadena formato”.
Una cadena formato es una cadena de texto que contiene escapes tales como % %p,
%S, y más. La salida resultante se crea tomando el texto para reemplazar los
escapes % de acuerdo a su significado (explicados a continuación).

Un escape % puede tener un tamaño definido, en cuyo caso siempre se reemplazaría
con la misma cantidad de texto (con espacios añadidos para rellenar si es
necesario), o puede ser “ampliable”, tomando el espacio que las columnas de
tamaño fijo no requieren. De existir varias columnas ampliables, el espacio se
distribuye de manera equitativa.

Todos los escapes % tienen un tamaño y/o capacidad de ampliación. Puede cambiar
el tamaño de un escape % insertándolo entre % y el carácter que identifica el
escape; por ejemplo, %20V genera la versión candidata del paquete, 20 caracteres
de ancho. El ancho “básico” de la columna puede variar dependiendo del contenido
si inserta un signo de interrogación (?) entre % y el carácter que identifica el
escape. Cabe que las columnas resultantes no se puedan alinear verticalmente.

Si desea poder ampliar un escape % en particular, a pesar de tener un ancho
definido, inserte una celdilla (p. ej., “#”) a su derecha. Por ejemplo, para
mostrar la versión candidata de un paquete sin importar su longitud, use la
cadena formato %V#. Puede también insertar # después de algo que no es un escape
%; aptitude “ampliará” el texto que precede a # añadiendo espacios tras él.
...
IMG.alt.suffix Nota
  = \[Nota]]
               Algunas de las descripciones a continuación se refieren al “paquete”. En la
               interfaz gráfica de usuario (GUI), esto es el paquete que está visionando o el
               seleccionado; en la busca en línea de órdenes, esto es el paquete que está
               visionando
...
%%        Literal %                        1                           No        Ésto no es realmente un escape; sólo inserta un signo porcentual en la salida en
                                                                                 el momento en que aparece.

                                                                                 En algunas circunstancias, una cadena formato de presentación puede tener
%#número Reemplazo de parámetro          Variable                    No        “parámetros”: por ejemplo, en el search de línea de órdenes, los grupos
                                                                                 encontrados en la búsqueda se usan como parámetros al presentar el resultado. El
                                                                                 parámetro, que se indica con número, reemplaza al código del formato.

                                                                                 Una marca de un solo carácter que resume cualquier acción que se va a ejecutar
%a        Marca de acción                 1                           No        sobre el paquete, como se describe en Figura 2.10, “Valores de la marca de
                                                                                 “acción””.

%A        Acción                          10                          No        Una descripción algo más detallada de la acción que se va a ejecutar sobre el
                                                                                 paquete.

%B        Total rotos                      12                          No        Si no hay paquetes rotos, no produce nada. De otra forma, genera una cadena como
                                                                                 por ejemplo “Broken: 10”, que describe el número de paquetes rotos.

%c        Marca de estado actual           1                           No        Una marca de un solo carácter que resume el estado actual del paquete, como se
                                                                                 describe en Figura 2.9, “Valores de la marca de “estado actual””.
...
%H        Nombre del anfitrión («host») 15                          No        El nombre del ordenador en el que ejecuta aptitude.
                                                                                 ...
                                                                                 Mostrar la prioridad más alta asignada a la versión de un paquete; para paquetes,
%i        Prioridad pin                    4                           No        muestra la prioridad de la versión que se va a instalar de forma predeterminada
                                                                                 (de existir).
...
%M        Marca de automático             1                           No        Si el paquete esta automáticamente instalado, da como salida “A”; si no, no
                                                                                 devuelve nada.

%n        Versión del programa            La longitud de “0.7.4”. No        Mostrar la versión de aptitude que está ejecutando, actualmente “0.7.4”.

%N        Nombre del programa              La longitud del nombre.     No        Mostrar el nombre del programa; generalmente, “aptitude”.

                                                                                 Si no se va a instalar ningún paquete, no muestra nada. De otra forma, muestra
%o        TamDescarga                      17                          No        una cadena que describe el tamaño total de todos los paquetes que va a instalar
                                                                                 (una estimación de cuanto necesita descargar); por ejemplo “TamDesc: 1000B”.

                                                                                 Mostrar el nombre del paquete. Cuando vea un paquete en un contexto de árbol, su
%p        Nombre del paquete               30                          Si        nombre estará en negrita, de ser posible, de acuerdo a su profundidad en el
                                                                                 árbol.
...
%R        Prioridad abreviada              3                           No        Mostrar una descripción abreviada de la prioridad de un paquete: por ejemplo
                                                                                 “Important” pasa a ser “Imp”.
...
%T        Etiqueta                         1                           No        Mostrar “*” si el paquete está etiquetado, de no ser así, no devuelve nada.[19]

                                                                                 Si las acciones seleccionadas van a alterar la cantidad de espacio usado en el
%u        Cambio de uso de disco           30                          No        disco, muestra la descripción del cambio en el espacio del disco duro; por
                                                                                 ejemplo “Se usará 100MB de espacio en disco.”

%v        Versión actual                  14                          No        Mostrar la versión instalada del paquete, o <none> si el paquete no está
                                                                                 instalado.

%V        Versión candidata               14                          No        Mostrar la versión del paquete que puede instalar si ejecuta Paquete → Instalar
                                                                                 (+) sobre el paquete, o <none> si el paquete no está disponible.

%Z        Cambio de espacio                9                           No        Mostrar cuanto espacio adicional se va a usar, o cuanto espacio se va a liberar
                                                                                 al instalar, actualizar o eliminar un paquete.
...
agrupación describe una “segmentación” de reglas; cada regla puede descartar
paquetes, crear sub-jerarquías en los cuales los paquetes residen, o manipular
el árbol. Los elementos de configuración Aptitude::UI::Default-Grouping y
Aptitude::UI::Default-Preview-Grouping definen las directrices de agrupación
para listas de paquetes recién creadas y pantallas de previsualización,
respectivamente. Puede configurar la directriz de agrupación para la lista de
paquetes actual pulsando G.
...
paquete con otra regla posterior. Si no se define una regla terminal, aptitude
utiliza la regla predeterminada, que es crear los “elementos de paquete”
estándar.
  ...
  pattern(patrón [=> título] [{ directriz }] [, ...])
      ...
      al mismo nivel que el agrupación patrón, en lugar de insertarlos en sub-
      árboles.
      ...
      acuerdo a las directrices que sigue pattern (patrón).
      ...
      Ejemplo 2.11. Uso de la directriz de agrupación pattern con sub-
      directrices.
      ...
      * Los paquetes que se van a eliminar se muestran en un sub-árbol
        etiquetado “Paquetes que se eliminarán”; la directriz de agrupación para
        este sub-árbol esta vacío, con lo cual se muestran los paquetes en una
        lista plana.
  ...
  section[(modo[,passthrough])]
      ...
      modo puede ser uno de los siguientes:
        ...
        subdirs
            ...
            primer signo /, si la porción del campo que lo antecede está dentro
            de la lista Aptitude::Sections::Top-Sections; si no, o en ausencia
            de /, se usará todo el campo. Si hay varios signos / en la porción
            del campo que esta en uso, se formará una jerarquía de grupos. Por
            ejemplo, si “games” no es un miembro de Aptitude::Sections::Top-
            Sections, entonces un paquete con una sección de “games/arcade” se
            colocará debajo de la cabecera de nivel superior “games”, en un sub-
            árbol llamado “arcade”.
  ...
  tag[(faceta)]
      ...
      Para más información acerca de debtags, consulte http://
      debtags.alioth.debian.org.
...
lista es el nombre de una regla de ordenación; si hay paquetes “iguales” de
acuerdo a la primera regla, se emplea la segunda regla para ordenarlos, y así en
adelante. Insertar un signo de tilde (~) delante de una regla revierte el
significado normal de esa regla. Por ejemplo, priority,~name ordena paquetes por
prioridad, pero los paquetes con la misma prioridad se colocarán en orden
inverso de acuerdo a su nombre.

Para modificar la directriz de ordenación en una lista de paquetes activa, pulse
S. Para modificar la ordenación predeterminada de todas las listas de paquetes,
configure la opción de configuración Aptitude::UI::Default-Sorting. Para
modificar la directriz de ordenación de búsqueda en aptitude, use la opción de
línea de órdenes --sort.
...
fichero de configuración. Cada orden tiene una variable de configuración
...
Search a “s”. Puede precisar que la tecla se debe pulsar «Control» introduciendo
“C-” delante de la tecla: por ejemplo, introducir “C-s” en vez de “s” ligaría la
búsqueda a Control+s en vez de a s. Por último, puede ligar la misma orden a
diferentes teclas de una sola vez usando una lista separada por comas: por
ejemplo, introducir “s,C-s” causaría que ambos s y Control+s ejecutasen una
búsqueda.
...
|______________________|______________|_________________________________________________________________________________|
|ApplySolution         |!             |Si hay paquetes rotos y aptitude ha sugerido una solución al problema, aplicar  |
|                      |              |inmediatamente la solución.                                                     |
|______________________|______________|_________________________________________________________________________________|
|Begin                 |home,C-a      |Desplazarse al inicio de la pantalla actual: a lo alto de una lista, o a la      |
|                      |              |izquierda de una entrada de texto en un campo.                                   |
|______________________|______________|_________________________________________________________________________________|
|BugReport             |B             |Informar de un fallo en el paquete seleccionado actualmente, empleando reportbug.|
|______________________|______________|_________________________________________________________________________________|
|Cancel                |C-g,escape,C-[|Cancelar la interacción actual: por ejemplo, descarta una ventana de dialogo o  |
|                      |              |desactiva el menú.                                                              |
|______________________|______________|_________________________________________________________________________________|
|Changelog             |C             |Mostrar el changelog.Debian (registro de cambios Debian) del paquete seleccionado|
|                      |              |o de la versión del paquete.                                                    |
|______________________|______________|_________________________________________________________________________________|
|ChangePkgTreeGrouping |G             |Modificar la directriz de agrupación de la lista de paquetes activa en ese      |
|                      |              |momento.                                                                         |
|______________________|______________|_________________________________________________________________________________|
...
|______________________|______________|_________________________________________________________________________________|
|CollapseTree          |Sin ligar     |Cerrar el árbol seleccionado en una lista jerárquica.                          |
|______________________|______________|_________________________________________________________________________________|
|                      |              |Esto equivale a pulsar “Ok” en los cuadros de dialogo; si está interactuando|con
|Confirm               |enter         |una pregunta de elección múltiple de la línea de estado, elige la opción     |
|                      |              |predeterminada.                                                                  |
|______________________|______________|_________________________________________________________________________________|
...
|______________________|______________|_________________________________________________________________________________|
|DescriptionCycle      |i             |Cuando examina la lista de paquetes, realiza un ciclo de las vistas disponibles  |
|                      |              |en el área de información.                                                     |
|______________________|______________|_________________________________________________________________________________|
|DescriptionDown       |z             |Cuando examina la lista de paquetes, desplaza el área de información una línea|
|                      |              |más abajo.                                                                      |
|______________________|______________|_________________________________________________________________________________|
|DescriptionUp         |a             |Cuando examina la lista de paquetes, desplaza el área de información una línea|
|                      |              |hacia arriba.                                                                    |
|______________________|______________|_________________________________________________________________________________|
|                      |              |Si no está en la pantalla de previsualización, muestra una pantalla de         |
|DoInstallRun          |g             |previsualización[a]; de estar viendo esta pantalla, ejecuta un proceso de       |
|                      |              |instalación.                                                                    |
|______________________|______________|_________________________________________________________________________________|
|Down                  |down,j        |Desplazarse hacia abajo: por ejemplo, desplaza el texto hacia abajo o selecciona |
|                      |              |el siguiente elemento de la lista.                                               |
|______________________|______________|_________________________________________________________________________________|
|DpkgReconfigure       |R             |Ejecutar “dpkg-reconfigure” sobre el paquete seleccionado.                   |
|______________________|______________|_________________________________________________________________________________|
|DumpResolver          |*             |De haber paquetes rotos, registra el estado actual del solucionador de problemas |
|                      |              |en un fichero (para corregir errores).                                           |
|______________________|______________|_________________________________________________________________________________|
|End                   |end,C-e       |Desplazarse al final de la pantalla actual: al final de la lista, o a la derecha |
|                      |              |de un campo de entrada de texto.                                                 |
|______________________|______________|_________________________________________________________________________________|
|ExamineSolution       |e             |Si hay paquetes rotos y aptitude ha sugerido una solución, muestra una ventana d|
|                      |              |dialogo con una descripción detallada de la solución propuesta.                |
|______________________|______________|_________________________________________________________________________________|
|ExpandAll             |[             |Expandir el árbol seleccionado y todas sus ramas en una lista jerárquica.      |
|______________________|______________|_________________________________________________________________________________|
|ExpandTree            |Sin ligar     |Expandir el árbol seleccionado en una lista jerárquica.                        |
|______________________|______________|_________________________________________________________________________________|
|FirstSolution         |<             |Selecciona la primera solución generada por el solucionador de problemas.       |
|______________________|______________|_________________________________________________________________________________|
|ForbidUpgrade         |F             |Prohibir que un paquete se actualice a la versión disponible (o a una versión e|
|                      |              |particular).                                                                     |
|______________________|______________|_________________________________________________________________________________|
|ForgetNewPackages     |f             |Descartar toda información relativa a qué paquetes son “nuevos” (vacía la |ista
|                      |              |de paquetes “Paquetes nuevos”).                                              |
|______________________|______________|_________________________________________________________________________________|
...
|______________________|______________|_________________________________________________________________________________|
|InstallSingle         |I             |Marcar un solo paquete para su instalación; todos los otros paquetes se         |
|                      |              |mantendrán en su versión actual.                                               |
|______________________|______________|_________________________________________________________________________________|
|Keep                  |:             |Cancelar cualquier petición de instalación o eliminación así como todas las  |
|                      |              |retenciones en un paquete.                                                       |
|______________________|______________|_________________________________________________________________________________|
|LastSolution          |<             |Selecciona la última solución generada por el solucionador de problemas.       |
|______________________|______________|_________________________________________________________________________________|
|Left                  |left,h        |Desplazarse a la izquierda: por ejemplo, mueve un menú a la izquierda de la barr|
|                      |              |de menú, o desplaza el cursor a la izquierda si edita texto.                    |
|______________________|______________|_________________________________________________________________________________|
|LevelDown             |J             |En una lista jerárquica, selecciona el siguiente hermano del elemento actualment|
|                      |              |seleccionado (el siguiente elemento del mismo nivel con la misma rama padre).    |
|______________________|______________|_________________________________________________________________________________|
|LevelUp               |K             |En una lista jerárquica, selecciona el hermano anterior al elemento seleccionado|
|                      |              |(el elemento anterior del mismo nivel con la misma rama padre).                  |
|______________________|______________|_________________________________________________________________________________|
|MarkUpgradable        |U             |Intenta actualizar todos los paquetes que no están retenidos o prohibidos de    |
|                      |              |actualización.                                                                  |
|______________________|______________|_________________________________________________________________________________|
...
|______________________|______________|_________________________________________________________________________________|
|MineUncoverSweepSquare|enter         |Descubrir la casilla presente en el Buscaminas en caso de estar oculta; de no    |
...
|______________________|______________|_________________________________________________________________________________|
|Parent                |^             |Selecciona la rama padre del elemento seleccionado en la lista jerárquica.      |
|______________________|______________|_________________________________________________________________________________|
...
|                      |              |Solucionador → Rechazar romper bloqueos.                                       |
|______________________|______________|_________________________________________________________________________________|
...
|______________________|______________|_________________________________________________________________________________|
|Right                 |right,l       |Desplazarse a la derecha: por ejemplo, desplazarse a un menú a la derecha, en la|
|                      |              |barra de menú, o desplazar el cursor a la derecha la editar texto.              |
|______________________|______________|_________________________________________________________________________________|
|Search                |/             |Activar la función de “búsqueda” del elemento de interfaz actualmente activ|.
|______________________|______________|_________________________________________________________________________________|
|SearchBack            |\             |Activar la función de “búsqueda inversa” del elemento de interfaz actualmen|e
|                      |              |activo.                                                                          |
|______________________|______________|_________________________________________________________________________________|
...
|______________________|______________|_________________________________________________________________________________|
|SolutionActionApprove |a             |Cuando examina una solución, marca la acción actualmente seleccionada como     |
|                      |              |«aprobada» (se incluirá en soluciones futuras, de ser posible).               |
|______________________|______________|_________________________________________________________________________________|
|SolutionActionReject  |r             |Cuando examina una solución, marca la solución actualmente seleccionado como   |
...
|______________________|______________|_________________________________________________________________________________|
|ToggleMenuActive      |C-m,f10,C-    |Activar o desactivar el menú principal.                                         |
|                      |space         |                                                                                 |
|______________________|______________|_________________________________________________________________________________|
|Undo                  |C-_,C-u       |Cancelar la última acción, hasta el punto en que inició aptitude, o hasta la  |
|                      |              |última vez que actualizó la lista de paquetes o instaló paquetes.             |
|______________________|______________|_________________________________________________________________________________|
|Up                    |up,k          |Desplazarse hacia arriba: por ejemplo, desplaza un texto arriba o selecciona el  |
|                      |              |elemento anterior en una lista.                                                  |
|______________________|______________|_________________________________________________________________________________|
|UpdatePackageList     |u             |Actualizar la lista de paquetes mediante la obtención de listas nuevas a través|
|                      |              |de Internet si es necesario.                                                     |
|______________________|______________|_________________________________________________________________________________|
...
Además de las teclas de letras, números y puntuación, puede ligar las siguientes
teclas “especiales”:
...
|____________|_______________________________________________________________|
|            |Coma (,) -- observe que debido a que las comas se emplean para |
|comma       |listar teclas, esta es la única manera de ligar una coma a una|
|            |acción.                                                       |
|____________|_______________________________________________________________|
...
|exit        |La tecla Salir.                                                |
|____________|_______________________________________________________________|
|f1, f2, ...,|Las teclas desde F1 a F10.                                     |
|f10         |                                                               |
|____________|_______________________________________________________________|
...
|EditLine     |Empleado por elementos de edición de línea, tales como el campo de entrada en|
|             |un dialogo de “búsqueda”.                                                 |
|_____________|_______________________________________________________________________________|
...
|_____________|_______________________________________________________________________________|
|MinibufChoice|Empleado por las preguntas de elección múltiple que aparecen si escoge que   |
|             |ciertas preguntas aparezcan en la línea de estado.                            |
|_____________|_______________________________________________________________________________|
|Pager        |Empleado cuando se muestra un fichero del disco (por ejemplo, el texto de      |
|             |ayuda).                                                                        |
|_____________|_______________________________________________________________________________|
|PkgNode      |Empleado por paquetes, árboles de paquetes, versiones de paquetes y           |
...
|_____________|_______________________________________________________________________________|
|TextLayout   |Empleado por presentaciones de texto formateados, tales como las descripciones |
|             |de paquetes.                                                                   |
|_____________|_______________________________________________________________________________|
|Tree         |Empleado por todas las presentaciones de árbol (incluyendo listas de paquetes,|
|             |se puede anular mediante PkgTree).                                             |
|_____________|_______________________________________________________________________________|
...
aptitude para mostrar el texto. Cada elemento visual tiene un “estilo” asociado,
que describe los colores y atributos visuales que se emplean para mostrar tal
elemento. Los estilos tienen la forma de una lista de configuraciones para el
color y los atributos. Esta lista no es necesariamente exhaustiva; si no se
define algún color o atributo sus valores se toman del contexto visual
circundante. De hecho, la mayoría de los elementos visuales tiene un estilo
“vacío” de manera predeterminada.

Puede modificar los contenidos de un estilo, creando un grupo de configuración
del mismo nombre en el fichero de configuración de apt o de aptitude. Por
ejemplo, el estilo de “MenuBorder” se utiliza para dibujar el borde en torno a
los menús desplegables. Por omisión este borde se dibuja resaltado y en blanco
sobre azul. Insertar el siguiente texto en el fichero de configuración lo dibuja
...
rojo, blanco y amarillo [20]. Además, puede definir default en lugar de un color
de fondo para usar el fondo predeterminado de la terminal (esto puede ser el
color predeterminado, un fichero de imagen, o incluso “transparente”). Los
estilos que aptitude reconoce son:
...
IMG.alt.suffix Nota
  = \[Nota]]
               Como se indica arriba, la interpretación del estilo y de los atributos de texto
               depende en gran medida de la terminal. Puede que necesite experimentar un poco
               para encontrar exactamente qué configuraciones son posibles en su terminal.
...
|__________________________|______________|______________________________________________________________________________|
|Bullet                    |fg yellow; set|El estilo de los puntos en las listas por puntos.                             |
|                          |bold;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|                          |              |El estilo de las nuevas versiones de un paquete en la vista de registro de    |
|ChangelogNewerVersion     |set bold;     |cambios. Observe que aptitude solo resalta las nuevas versiones de paquetes si|
|                          |              |el paquete «libparse-debianchangelog-perl» está instalado.                 |
|__________________________|______________|______________________________________________________________________________|
|Default                   |fg white; bg  |El estilo básico de la pantalla.                                             |
|                          |black;        |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|DepBroken                 |fg black; bg  |El estilo para las dependencias no satisfechas.                               |
|                          |red;          |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|DisabledMenuEntry         |fg black; bg  |El estilo para las entradas de menú que están desactivadas y que no se puede|
|                          |blue; set dim;|emplear.                                                                      |
|__________________________|______________|______________________________________________________________________________|
|DownloadHit               |fg black; bg  |El estilo empleado para indicar que un archivo ha sido “hit”: p.ej., no ha|
|                          |green;        |cambiado desde la última vez que se descargó.                               |
|__________________________|______________|______________________________________________________________________________|
|DownloadProgress          |fg blue; bg   |El estilo del indicador de progreso de una descarga.                          |
|                          |yellow;       |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|                          |fg white; bg  |El estilo de editores de línea (por ejemplo, la entrada en el cuadro de      |
|EditLine                  |black; clear  |dialogo “Buscar”).                                                        |
|                          |reverse;      |                                                                              |
|__________________________|______________|______________________________________________________________________________|
...
|__________________________|______________|______________________________________________________________________________|
|                          |fg white; bg  |                                                                              |
|Header                    |blue; set     |El estilo de las cabeceras de pantalla.                                       |
|                          |bold;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|                          |fg white; bg  |                                                                              |
|HighlightedMenuBar        |blue; set     |El estilo del nombre de menú seleccionado en la barra de menú.              |
|                          |bold,reverse; |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|                          |fg white; bg  |                                                                              |
|HighlightedMenuEntry      |blue; set     |El estilo de la elección seleccionada actualmente en un menú .              |
|                          |bold,reverse; |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|MediaChange               |fg yellow; bg |El estilo del cuadro de dialogo empleado para informar al usuario de que debe |
|                          |red; set bold;|insertar un disco compacto nuevo.                                             |
|__________________________|______________|______________________________________________________________________________|
|                          |fg white; bg  |                                                                              |
|MenuBar                   |blue; set     |El estilo de la barra de menú.                                               |
|                          |bold;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|                          |fg white; bg  |                                                                              |
|MenuBorder                |blue; set     |El estilo de los bordes que rodean a un menú desplegable.                    |
|                          |bold;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|MenuEntry                 |fg white; bg  |El estilo de cada entrada en un menú desplegable.                            |
|                          |blue;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|MineBomb                  |fg red; set   |El estilo de las bombas en el Buscaminas.                                     |
|                          |bold;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|MineBorder                |set bold;     |El estilo del borde que se dibuja en torno al tablero del Buscaminas.         |
|__________________________|______________|______________________________________________________________________________|
|MineFlag                  |fg red; set   |El estilo de las marcas en el Buscaminas.                                     |
|                          |bold;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|MineNumberN               |Varios        |El estilo del número N en el Buscaminas; N puede variar de 0 a 8.            |
|__________________________|______________|______________________________________________________________________________|
|MultiplexTab              |fg white; bg  |El color empleado para mostrar “pestañas” además de la seleccionada.    |
|                          |blue;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|MultiplexTabHighlighted   |fg blue; bg   |El color empleado para mostrar la “pestaña” seleccionada.                |
|                          |white;        |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|PkgBroken                 |fg red; flip  |El estilo de los paquetes en la lista de paquetes que tienen dependencias no  |
|                          |reverse;      |satisfechas.                                                                  |
|__________________________|______________|______________________________________________________________________________|
|PkgBrokenHighlighted      |fg red;       |El estilo de los paquetes resaltados en la lista de paquetes que tienen       |
|                          |              |dependencias no satisfechas.                                                  |
|__________________________|______________|______________________________________________________________________________|
|PkgNotInstalled           |             |El estilo de los paquetes que no están instalados y que no se van a instalar.|
|__________________________|______________|______________________________________________________________________________|
|PkgNotInstalledHighlighted|             |El estilo de los paquetes resaltados que no están instalados y que no se van |
|                          |              |instalar.                                                                     |
|__________________________|______________|______________________________________________________________________________|
|PkgIsInstalled            |set bold;     |El estilo de los paquetes instalados en la actualidad pero para los cuales no |
|                          |              |hay ninguna acción programada.                                               |
|__________________________|______________|______________________________________________________________________________|
|PkgIsInstalledHighlighted |set bold; flip|El estilo de los paquetes resaltados instalados en la actualidad pero para los|
|                          |reverse;      |cuales no hay ninguna acción programada.                                     |
|__________________________|______________|______________________________________________________________________________|
|PkgToDowngrade            |set bold;     |El estilo de los paquetes en la lista de paquetes que se van a desactualizar. |
|__________________________|______________|______________________________________________________________________________|
|PkgToDowngradeHighlighted |set bold; flip|El estilo de los paquetes resaltados en la lista de paquetes que se van a     |
|                          |reverse       |desactualizar.                                                                |
|__________________________|______________|______________________________________________________________________________|
|PkgToHold                 |fg white; flip|El estilo de los paquetes retenidos en la lista de paquetes.                  |
|                          |reverse;      |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|PkgToHoldHighlighted      |fg white;     |El estilo de los paquetes retenidos, y resaltados, en la lista de paquetes.   |
|__________________________|______________|______________________________________________________________________________|
|PkgToInstall              |fg green; flip|El estilo de los paquetes en la lista de paquetes que se van a instalar (no   |
|                          |reverse;      |actualizar) o reinstalar.                                                     |
|__________________________|______________|______________________________________________________________________________|
|PkgToInstallHighlighted   |fg green;     |El estilo de los paquetes resaltados en la lista de paquetes que se van a     |
|                          |              |instalar (no actualizar) o reinstalar.                                        |
|__________________________|______________|______________________________________________________________________________|
|PkgToRemove               |fg magenta;   |El estilo de los paquetes en la lista de paquetes que se van a eliminar o     |
|                          |flip reverse; |purgar.                                                                       |
|__________________________|______________|______________________________________________________________________________|
|PkgToRemoveHighlighted    |fg magenta;   |El estilo de los paquetes resaltados en la lista de paquetes que se van a     |
|                          |              |eliminar o purgar.                                                            |
|__________________________|______________|______________________________________________________________________________|
|PkgToUpgrade              |fg cyan; flip |El estilo de los paquetes en la lista de paquetes que se van a actualizar.    |
|                          |reverse;      |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|PkgToUpgradeHighlighted   |fg cyan;      |El estilo de los paquetes resaltados en la lista de paquetes que se van a     |
|                          |              |actualizar.                                                                   |
|__________________________|______________|______________________________________________________________________________|
|Progress                  |fg blue; bg   |El estilo de las barras de progreso tales como el que aparece cuando se carga |
|                          |yellow;       |el almacén de paquetes.                                                      |
|__________________________|______________|______________________________________________________________________________|
...
|__________________________|______________|______________________________________________________________________________|
|                          |fg white; bg  |                                                                              |
|Status                    |blue; set     |El estilo de las líneas de estado en la base de la pantalla.                 |
|                          |bold;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
|TreeBackground            |             |El color básico de todas las listas y árboles.                              |
|__________________________|______________|______________________________________________________________________________|
|                          |fg red; bg    |El color empleado para mostrar los avisos referentes a la confianza de los    |
|TrustWarning              |black; set    |paquetes.                                                                     |
|                          |bold;         |                                                                              |
|__________________________|______________|______________________________________________________________________________|
...
presentes; determinan la posición del elemento de pantalla (consulte más abajo
...
izquierda de un elemento está en la célula dada por las opciones Row y Column
(comenzando generalmente por la fila 0, columna 0; esto no es obligatorio). El
ancho de un elemento en células aparece en la opción Width, y su altura, en la
opción Height.
...
llamado “Header”, con un ancho de tres células y que se expande horizontalmente,
pero no en vertical. Posee el mismo color que otras líneas de cabecera y emplea
el formato de presentación estándar para las líneas de cabecera:
...
En su forma básica, el fichero de configuración de aptitude es una lista de
opciones seguidas de sus valores. Cada línea debe tener la forma “Opción
Valor;”: por ejemplo, la siguiente línea en el fichero de configuración
configura la opción Aptitude::Theme como “Dselect”.

Aptitude::Theme "Dselect";

Una opción puede “contener” otras opciones si se escriben entre corchetes entre
la opción y el semi-colon que le sigue,como por ejemplo:
...
Para más información referente al formato del fichero de configuración, consulte
la página de manual apt.conf(5).
...
La configuración de aptitude se lee desde las siguientes fuentes, en orden:
  ...
  1. Opciones del fichero de configuración definidas en la línea de órdenes.

  2. El fichero de configuración de usuario, ~/.aptitude/config. Este fichero se
     sobreescribe cuando el usuario modifica la configuración en el menú
     Opciones.

  3. El fichero de configuración del sistema, /etc/apt/apt.conf.

  4. Los fragmentos de fichero de configuración del sistema, /etc/apt/
     apt.conf.d/*.
...
Cuando se activa una opción, estas fuentes se examinan por orden, y se emplea el
primero que provee el valor para la opción usada. Por ejemplo, si configura una
opción en /etc/apt/apt.conf, esto sobreescribe los valores predeterminados de
aptitude para esa opción, pero no sobreescribe las configuraciones de usuario en
~/.aptitude/config.
...
valga false. Para más información, consulte “Gestionar paquetes automáticamente
instalados.”.
...
Descripción: Si esta opción tiene valor de true, aptitude considerará que ningún
paquete está en desuso (y por ello no los eliminará automáticamente) siempre y
cuando algún paquete los sugiera. Para más información, consulte “Gestionar
paquetes automáticamente instalados.”.
...
opciones como false, aptitude no borrará los ficheros antiguos de listas de
...
estas opciones como false, aptitude no borrará los ficheros antiguos de listas
...
también, cuando marque un paquete para su instalación aptitude marcará también
todos los paquetes recomendados. Más aún, si esta opción es true, aptitude
considerará que ninguna paquete está en desuso mientras otro lo recomiende. Para
más información, consulte “Gestionar paquetes automáticamente instalados.” y
“Resolución inmediata de dependencias.”.
...
volverá a la lista de paquetes. Si esta opción vale true, aptitude mostrará la
pantalla de previsualización si hay paquetes actualizables, en lugar de mostrar
un recordatorio relativo a la orden Acciones → Marcar actualizable (U).
...
aptitude siempre emplearán un solucionador de dependencias “seguro”, al igual
que si hubiese introducido el argumento --safe-resolver en la línea de órdenes.

Opción:Aptitude::Autoclean-After-Update

Predeterminado:false

Descripción: Si esta opción tiene valor de true, aptitude borrará los paquetes
obsoletos (consulte Acciones → Limpiar ficheros obsoletos) cada vez que
...
Descripción: Si esta opción es true, aptitude intentará automáticamente cumplir
con las dependencias de un paquete cuando lo marque para su instalación o
actualización.
...
instalación o actualización, Generalmente se marcan estos conflictos, y tendrá
...
Descripción: Si esta opción es true, aptitude marcará automáticamente todos los
paquetes actualizables cuando inicie el programa, al igual que si ejecuta la
...
si la pregunta se obviaría en situaciones normales. Esto equivale a la opción en
línea de órdenes -P.
...
actuará como si el usuario hubiese respondido “si” a cada pregunta, haciendo que
la mayoría de ellos se obvien. Esto equivale a la orden en línea de órdenes -y.

Opción:Aptitude::CmdLine::Disable-Columns

Predeterminado:false

Descripción: Si se activa esta opción, los resultados de una búsqueda en la
línea de órdenes (ejecutados con aptitude search) no se formatearán en columnas
de tamaño fijo o cortados al ancho de la pantalla. Esto equivale a la opción en
la línea de órdenes --disable-columns.

Opción:Aptitude::CmdLine::Download-Only

Predeterminado:false

Descripción: En el modo de línea de órdenes, si esta opción vale true, aptitude
descargará los paquetes pero no los instalará. Esto equivale a la opción de
línea de órdenes -d.
...
Descripción: En el modo de línea de órdenes, si esta opción vale true, aptitude
será más agresivo en sus intentos de arreglar las dependencias de un paquete
roto. Esto equivale a la opción en la línea de órdenes -f.

Opción:Aptitude::CmdLine::Versions-Group-By

Predeterminado: Se puede definir con auto, none, package, o source-package para
si se muestra la salida de aptitude versions, y la forma en que se agrupa.
Equivale a la opción de línea de órdenes --group-by (consulte la documentación
para una mejor descripción del significado de los valores).
...
Descripción: Esta es una cadena formato, como se describe en “Personalizar la
presentación de los paquetes”, que se emplea para mostrar los resultados de una
búsqueda en la línea de órdenes. Esto equivale a la opción de línea en órdenes -
F.
...
Descripción: Esta opción proporciona el ancho de los caracteres que aparecen en
una búsqueda en línea de órdenes. De estar vacía (predeterminado; p.ej., ""),
los resultados de búsqueda se formatearán en relación al tamaño de la terminal,
o para una exposición de 80 columnas si no se puede determinar el tamaño de la
terminal.
...
Descripción: Esta opción controla si los indicadores de progreso de línea de
órdenes muestran el porcentaje en la parte izquierda de la pantalla, al igual
que apt-get, o en la parte derecha (predeterminado). Esta opción no afecta a los
...
Descripción: Si este valor se define como false, los indicadores de progreso de
línea de órdenes se eliminarán y sobreescribirán una vez que la tarea que
representan haya finalizado. Si es true, permanecerán en la terminal. Esta
opción no afecta a los indicadores de progreso de descarga.
...
Descripción: En el modo de línea de órdenes, si esta opción es true, aptitude
mostrará información extremadamente detallada cuando intente resolver
dependencias rotas. Como su propio nombre sugiere, esta opción está pensada para
ayudar en el proceso de eliminar los fallos del solucionador.

Opción:Aptitude::CmdLine::Resolver-Dump

Predeterminado:

Descripción: En el modo de línea de órdenes, en caso de ser necesario solucionar
dependencias rotas y si esta opción se configura con el nombre de un fichero con
permisos de escritura, el estado del solucionador se guardará en este fichero
antes de realizar cualquier calculo.

Opción:Aptitude::CmdLine::Resolver-Show-Steps

Predeterminado:false

Descripción: Si esta opción vale true, se mostrará una solución de dependencias
como una secuencia de las resoluciones de las dependencias individuales; por
ejemplo “wesnoth depende de wesnoth-data (= 1.2.4-1) -> instalar wesnoth-data
1.2.4-1 (unstable)”. Para conmutar el modo de vista, pulse o cuando se le
pregunte “¿Acepta esta solución?”.
...
Descripción: En el modo de línea de órdenes, si esta opción vale true, aptitude
mostrará un resumen de las dependencias (de haberlas) relacionadas con el estado
de un paquete. Esto equivale a -D en la línea de órdenes.
...
Descripción: En modo de línea de órdenes, si esta opción vale true, aptitude
mostrará una estimación del espacio que usará cada paquete. Esto equivale a la
opción de línea de órdenes -Z.

Opción:Aptitude::CmdLine::Show-Summary

Predeterminado:no-summary

Descripción: Esta opción define el valor predeterminado del argumento en línea
de órdenes --show-summary. Véase la documentación de --show-summary para una
lista de los valores permitidos con esta opción, así como su significado.
...
Descripción: En modo de línea de órdenes, si esta opción vale true, aptitude
mostrará la versión del paquete que se va a instalar o eliminar. Esto equivale a
la opción en línea de órdenes -V.
...
opción en línea de órdenes -W, y muestra la misma información accesible a través
de aptitude why, o pulsando i en la lista de paquetes.
...
Descripción: Esta es una cadena formato, como se describe en “Personalizar la
presentación de los paquetes”, que se emplea para mostrar la salida de aptitude
versions. Esto equivale a la opción de línea en órdenes -F.

Opción:Aptitude::CmdLine::Versions-Show-Package-Names

Predeterminado: Se puede definir con always, auto, o never para controlar cuándo
se muestran nombres de paquete en la salida de aptitude versions. Equivale a la
opción en línea de órdenes --show-package-names (consulte su documentación para
una descripción más amplia del significado de los valores).
...
dependencias “safe” mediante --safe-resolver o porque se utiliza la opción de
línea de órdenes safe-upgrade, mostrará un resumen de las acciones que el
solucionador tomará antes de previsualizar la instalación. Equivale a la opción
de línea de órdenes --show-resolver-actions.

Opción:Aptitude::Screenshot::IncrementalLoadLimit

Predeterminado:16384

Descripción: El tamaño mínimo en bytes en el que aptitude empezará a mostrar las
capturas de pantalla de forma incremental. Las capturas de pantalla menores que
este tamaño no se mostrarán hasta que se haya completado su descarga.

Opción:Aptitude::Screenshot::Cache-Max

Predeterminado:4194304

Descripción: El tamaño máximo en bytes de datos de capturas de pantalla que
aptitude guardará para aquellas capturas que no se están mostrando actualmente.
El valor predeterminado es de cuatro megabytes.
...
Descripción: Controlar cuanta información recibe en modo de línea de órdenes de
aptitude. Cada aparición de la opción -v añade 1 a este valor.

Opción:Aptitude::CmdLine::Visual-Preview

Predeterminado:false

Descripción: If this option is true, aptitude will enter its visual interface to
...
Descripción: Si se activa esta opción, se eliminarán los paquetes instalados
automáticamente que ya no se requieran. Para más información, consulte
“Gestionar paquetes automáticamente instalados.”.
...
Descripción: Si esta opción vale true, aptitude mostrará una previsualización
antes de ejecutar las acciones que desea llevar a cabo.
...
Descripción: Esta opción define la orden externa que aptitudeutiliza para pasar
a usuario root (consulte “Convertirse en root.”). Su forma es protocolo:orden.
El protocolo debe ser su o sudo; define la manera en que aptitude invoca el
programa cuando desea obtener privilegios de usuario root. Si el protocolo es
su, se emplea la orden -c argumentos para convertirse en el usuario root; de no
ser así, aptitude utiliza orden argumentos. La primera palabra de la orden es el
nombre del programa que invocar; las palabras restantes se toman como argumentos
...
vale true, aptitude le preguntará si desea eliminar esta carpeta. La opción será
true automáticamente tras su respuesta. Por otro lado, si la carpeta no existe,
esta opción hará que aptitude tome esto como false para así notificarle en caso
de que reaparezca.
...
Descripción: En versiones anteriores de aptitude, la opción Aptitude::
Recommends-Important activaba la instalación automática de paquetes
recomendados, de la misma manera que hoy en día realiza APT::Install-Recommends.
Si esta opción se define como false, y Aptitude::Recommends-Important también se
define como false, aptitude configurará APT::Install-Recommends como false, y
...
Descripción: This is an obsolete option; use APT::AutoRemove::
...
Descripción: This is an obsolete option; use APT::AutoRemove::SuggestsImportant
...
paquetes que no se correspondan con el patrón (consulte “Patrones de búsqueda”).
Si define esta opción como una cadena vacía (opción predeterminada), se eliminan
todos aquellos paquetes no usados.

Opción:Aptitude::LockFile

Predeterminado:/var/lock/aptitude

Descripción: Un fichero que estará «fcntl-locked» para asegurar que sólo un
proceso de aptitude pueda modificar el almacén en cada momento. Nunca tendrá la
necesidad de modificar esto en circunstancias normales; aunque puede ser útil
para depurar fallos. Nota: si aptitude informa que no puede conseguir el permiso
único sobre el fichero, esto no significa que necesite destruir el fichero. Los
«fcntl locks» (cerrojos) se gestionan por el kernel y son destruidos cuando el
programa que los emplea finaliza; un fallo en el momento de adquirir el permiso
...
el resto de su valor se emplea como el nombre de una orden al cual se redirigirá
el registro: por ejemplo, |mail -s 'Aptitude install run' root permite mandar el
registro al root por correo electrónico. Puede definir esta opción como una
lista de objetivos de ficheros de registro para registrar varios ficheros u
órdenes,.
...
escribirá en él los mensajes que vaya registrando; si lo define con “-”, se
mostrarán los mensajes a través de la salida estándar. Esto se diferencia de la
configuración Aptitude::Log: ese fichero se utiliza para registrar las
instalaciones y eliminaciones, mientras que éste se utiliza para registrar
eventos de programas, errores y mensajes de depuración de fallos (si está
activado). Esta opción equivale al argumento de línea de órdenes --log-file.
Véase también Aptitude::Logging::Levels.
...
mensajes del registro se escriben. Cada entrada es, o bien “nivel”, para definir
el nivel de registro global (el nivel de registro del registrador del
administrador) con el “nivel” insertado, o bien “categoría:nivel”, donde
categoría es la categoría de los mensajes que modificar (tales como
aptitude.resolver.hints.match), y nivel es el nivel más bajo de registro de
mensajes en esa categoría que se deberían ver. Los niveles de registro válidos
son “fatal”, “error”, “warn” (aviso), “info” (información), “debug” (depuración
de fallos), y “trace” (rastro). Puede emplear la opción en línea de órdenes --
log-level para definir o invalidar cualquier nivel de registro.
...
consulte “Patrones de búsqueda” para ver los detalles acerca de su formato.
...
activarlas manualmente (consulte “Resolver dependencias de manera
interactiva.”).
...
Descripción: Cuánto premiar o penalizar soluciones que modifican el estado de un
paquete retenido o que instalan versiones prohibidas. Observe que a menos que
...
Descripción: El coste de seguridad asignado a acciones que rompen una retención
creada por el usuario (p. ej, actualizar un paquete retenido, o instalar una
versión prohibida). Para una descripción del funcionamiento de los costes de
...
Descripción: Cuánto premiar o penalizar soluciones potenciales en base al número
de dependencias que rompen. Este número se añadirá a la puntuación de cada
solución por cada dependencia rota que genere; por lo general, éste tiene un
valor negativo.
...
Descripción: Cuánto premiar o penalizar una solución potencial en base a cuantas
resoluciones “predeterminadas” instalan por dependencias actualmente
insatisfechas. La resolución predeterminada es la que tomaría “apt-get install”
o el “solucionador de dependencias inmediato”. La puntuación sólo se aplica a
las dependencias y recomendaciones cuyos objetivos no están instalados
...
Descripción: El coste de seguridad asignado a acciones que eliminan un paquete
marcado como «Essential» (esencial). Para una descripción del funcionamiento de
...
Descripción: Este número se añadirá a la puntuación de cualquier paquete con
prioridad “extra”.
...
Descripción: Este número se añade al puntuación de una solución que elimina un
paquete e instala otro que lo reemplaza por completo (p. ej., tiene un
...
Descripción: Cuántos “pasos” debe tomar el solucionador antes de encontrar la
primera solución. Aunque aptitude intenta generar las mejores soluciones antes
que las peores es a veces incapaz de ello; esta configuración permite al
solucionador buscar brevemente una solución mejor antes de mostrar los
resultados, a diferencia de detenerse inmediatamente tras encontrar la primera
solución.
...
que describe la acción que se debería aplicar a uno o más paquetes. La sintaxis
para cada indicación y el efecto que tiene se pueden ver en “Configurar
indicaciones del solucionador”.

Opción:Aptitude::ProblemResolver::ImportantScore

Predeterminado:5

Descripción: Este número se añade a la puntuación de cualquier versión de un
paquete con una prioridad “importante”.

Opción:Aptitude::ProblemResolver::Infinity

Predeterminado:1000000

Descripción: La puntuación “máxima” para soluciones en potencia. Si un conjunto
de acciones tiene una puntuación peor que -Infinity, se descartará
inmediatamente.
...
Descripción: El coste de seguridad asignado a la única solución que cancela
todas las acciones seleccionadas por el usuario.Para una descripción del
...
una versión no predeterminada de un paquete (una que no es la versión presente y
tampoco la “versión candidata”).

Opción:Aptitude::ProblemResolver::Non-Default-Level

Predeterminado:50000

Descripción: El coste de seguridad asignado a acciones que instalan versiones no
predeterminadas de un paquete. Por ejemplo, su la versión 5 de un paquete se
encuentra instalada, están disponibles las versiones 6,7 y 8, y la versión
predeterminada es la 7, las versiones 6 y 8 recibirían un coste de seguridad al
menos tan alto como esto. Para una descripción del funcionamiento de los costes
de seguridad consulte “Costes de seguridad”.

Opción:Aptitude::ProblemResolver::OptionalScore

Predeterminado:1

Descripción: Este número se añade a la puntuación de toda versión de un paquete
...
Descripción: Cuánta importancia otorga el solucionador de problemas a conservar
las instalaciones automáticas o eliminaciones.
...
Descripción: El coste de seguridad que se asigna a acciones que eliminan un
paquete. Para una descripción del funcionamiento de los costes de seguridad
...
Descripción: El coste de seguridad que se asigna a acciones que instalan la
versión predeterminada de un paquete, actualizan un paquete a su versión
predeterminada, o cancelan instalar o actualizar un paquete. Las soluciones que
reciben este coste también se pueden generar mediante aptitude safe-upgrade.
Para una descripción del funcionamiento de los costes de seguridad consulte
“Costes de seguridad”.
...
Descripción: Describe como determinar el coste de una solución. Consulte “Costes
en el solucionador interactivo de dependencias.” para una descripción de los
costes de una solución, lo que hacen y la sintaxis para definirlas. Si el coste
no se puede analizar, se muestra un error y en su lugar se utiliza el coste
predeterminado.
...
Descripción: Este número se añade a la puntuación de toda versión de un paquete
con prioridad “estándar”.

Opción:Aptitude::ProblemResolver::StepLimit

Predeterminado:5000

Descripción: El número máximo de “pasos” que debería tomar el solucionador de
problemas a cada intento de encontrar una solución a un problema de
dependencias. Si reduce este número, aptitude se dará antes “por vencido”; si lo
aumenta, permitirá que la búsqueda de una solución pueda tomar más tiempo y
memoria antes de interrumpirse. Si define el valor de StepLimit como 0,
desactivaría totalmente el solucionador de problemas. El valor predeterminado es
suficientemente alto para gestionar cómodamente las situaciones más comunes, a
la vez que impide que aptitude “explote” en caso de encontrarse con un problema
más complicado. (Nota: ésto sólo afecta a las búsquedas en línea de órdenes; en
la interfaz gráfica el solucionador continuará hasta encontrar una solución)
...
solución se genere más rápidamente, pero ésta puede ser de menor calidad que en
...
fichero de seguimiento («trace-file»). Los directorios de seguimiento son más
transparentes que los ficheros de seguimiento, y más adecuados para, por
...
genere una solución guardará una versión simplificada del estado del paquete,
que se puede emplear para reproducir la misma solución. Si define también
Aptitude::ProblemResolver::Trace-Directory, se escribirá la misma información en
el directorio de seguimiento («trace-directory»). Un fichero de seguimiento es
simplemente un directorio de seguimiento comprimido en un fichero; ocupa menos
espacio que el directorio de seguimiento y es más apropiado para su transmisión
a través de una red.
...
«Recomienda». Generalmente, este número es menor que «RemoveScore», o aptitude
tenderá a eliminar paquetes antes que dejar sus recomendaciones sin resolver.
Para más detalles, consulte “Resolver dependencias de manera interactiva.”.

Opción:Aptitude::ProblemResolver::UpgradeScore

Predeterminado:0

Descripción: Cuánta importancia otorga el solucionador de problemas a actualizar
(o desactualizar) un paquete a su versión candidata, en caso de que el paquete
...
eliminando sus datos de configuración así como quizás otros datos importantes.
Para más información relativa a qué paquetes se consideran “no usados”, consulte
“Gestionar paquetes automáticamente instalados.”. ¡ÉSTA OPCIÓN PUEDE CAUSAR
PÉRDIDA DE DATOS! ¡NO LA HABILITE A MENOS QUE SEPA LO QUE ESTÁ HACIENDO!
...
Recommends-Important en su fichero de configuración de usuario.
...
solucionador de dependencias “seguro” mediante --safe-resolver o utilizando la
opción de línea de órdenes safe-upgrade, no se permitirá al solucionador
instalar paquetes que no lo están ya actualmente.
...
dependencias “seguro” mediante --safe-resolver o utilizando la opción de línea
de órdenes safe-upgrade, no se permitirá al solucionador instalar paquetes que
no lo están ya actualmente.
...
mostradas para cada sección cuando emplea la directriz de agrupación “section”
para la jerarquía de paquetes. Las descripciones se asignan a árboles de sección
en base al último componente del nombre: por ejemplo, un miembro del grupo
llamado “games” se utilizará para describir las secciones “games”, “non-free/
games”, y “non-free/desktop/games”. En el texto comprendido en las descripciones
de sección, se reemplaza la cadena “\n” por un corte de líneas, y la cadena
...
Descripción: Un grupo de configuración cuyos elementos son los nombres de las
secciones en el nivel superior del archivo. Las directrices de agrupación
“topdir”, “subdir”, y “subdirs” utilizan esta lista para interpretar los campos
de «Sección»: si el primer elemento de ruta de la sección de un paquete no está
contenido en esta lista, o si su sección sólo tiene un elemento, se agrupará el
paquete utilizando el primer miembro de esta lista como primer elemento de ruta.
Por ejemplo, si el primer miembro de Top-Sections es “main”, un paquete cuya
sección es “games/arcade” se tratará como si su campo de sección fuese “main/
games/arcade”.
...
independientemente de si usted es root (administrador), o no. Ésto equivale a la
opción en línea de órdenes -s.

Opción:Aptitude::Spin-Interval

Predeterminado:500

Descripción: El número de segundos que dejar entre actualizar la “rueda” que
...
vez que intente modificar el estado de los paquetes cuando aptitude está en modo
de sólo lectura.
...
Descripción: El tema que aptitude debe utilizar; para más información, consulte
“Temas.”.
...
Descripción: Si define esta opción como true, aptitude intentará detectar si se
ha modificado el estado de un paquete con dselect o con la orden dpkg: por
...
que parece causar que otros paquetes estén rotos el área de información mostrará
algunas de las razones de porqué la ruptura tiene lugar.
...
Descripción: Definir la directriz de agrupación predeterminada utilizada en las
listas de paquetes. Para información adicional acerca de las directrices de
agrupación, consulte “Personalizar la jerarquía de paquetes”.
...
predeterminada de la interfaz de aptitude. Para más información, consulte
“Personalizar el diseño de la interfaz.”.
...
directrices de agrupación, consulte “Personalizar la jerarquía de paquetes”.
...
cierra aptitude; de no ser así aptitude no se cerrará hasta que invoque la orden
Acciones → Salir (Q). Para más información, consulte “Trabajar con varias
vistas.”.
...
Descripción: Si define esta opción como true, aptitude le dará un formato a las
descripciones de manera que cada línea ocupe exactamente el ancho de la
pantalla.
...
Descripción: Si define esta opción como true, aptitude mostrará una vista plana
al inicio, en lugar de la vista predeterminada.

Opción:Aptitude::UI::HelpBar

Predeterminado:true

Descripción: Si define esta opción como true, verá una línea con los atajos de
teclado más importantes en la parte superior de la pantalla.
...
siguiente paquete que se corresponda con lo que ha introducido hasta el momento.

Opción:Aptitude::UI::InfoAreaTabs

Predeterminado:false

Descripción: Si define esta opción como true, aptitude mostrará pestañas justo
encima del área de información (el panel en el fondo de la pantalla),
...
Descripción: Éste es un grupo cuyos miembros definen las conexiones entre atajos
de teclado y órdenes dentro de aptitude. Para más información, consulte
“Personalizar teclas rápidas.”.
...
preguntas yes/no o preguntas de elección múltiple) se mostrarán en la base de la
pantalla, y no en ventanas de dialogo.
...
Instalar (+) tendrán el mismo comportamiento, ya obsoleto, que tenían en
versiones anteriores de aptitude.
...
paquetes en la lista de paquetes. Para más información, consulte “Personalizar
la presentación de los paquetes”.
...
formato, consulte “Personalizar la presentación de los paquetes”.
...
Descripción: Esta opción controla la cadena formato utilizada para mostrar la
línea de estado de las listas de paquetes (p. ej., la línea que aparece entre la
lista de paquetes y el área de información). Para más información relativa a las
cadenas formato, consulte “Personalizar la presentación de los paquetes”.

Opción:Aptitude::UI::Pause-After-Download

Predeterminado:OnlyIfError

Descripción: Si define esta opción como true, aptitude le preguntará si quiere
continuar con la instalación una vez finalizada la descarga. Si el valor es
OnlyIfError, el mensaje solo aparecerá en caso de que la descarga haya fallado.
De otra manera, si la opción tiene valor de false aptitude procederá
...
previsualización. Para más detalles acerca de su formato, consulte “Patrones de
búsqueda”.
...
Descripción: Éste es un grupo de configuración cuyos miembros definen el estilo
de texto que aptitude emplea para mostrar información. Para más información,
consulte “Personalizar los colores del texto y estilos.”.
...
“pestañas” describiendo las vistas activas presentes en el margen superior de la
pantalla.
...
Descripción: Si define esta opción con valor de true, aptitude detectará cuando
precise privilegios de root (administrador), y le preguntará si desea cambiar a
la cuenta de root si aún no lo es. Para más información, consulte “Convertirse
en root.”.
...
Descripción: La ubicación del fichero de vocabulario de debtags, utilizado para
cargar los meta-datos de marcas de paquetes.
...
En aptitude un tema es simplemente un conjunto de configuraciones que “están
agrupados”. Los temas funcionan invalidando los valores predeterminados de las
opciones: si una opción no está definida en el fichero de configuración del
sistema, o en su propio fichero personal de configuración, aptitude utilizará la
configuración del tema presente de haber uno disponible, antes de utilizar el
valor estándar predeterminado.

Un tema es simplemente un grupo nombrado bajo Aptitude::Themes; cada opción de
configuración contenida en el grupo invalidará la correspondiente opción en la
configuración global. Por ejemplo, si selecciona el tema Dselect, la opción
...
Para seleccionar un tema, defina la opción de configuración Aptitude::Theme con
el nombre del tema; por ejemplo,

Aptitude::Theme Vertical-Split;

aptitude incorpora los siguientes temas en /usr/share/aptitude/aptitude-
defaults:
...
En el caso de que esté cansado de instalar y eliminar paquetes, aptitude incluye
una versión del clásico juego “Buscaminas”. Seleccione Acciones → Jugar al
buscaminas para iniciarlo; verá entonces el tablero inicial del Buscaminas:
...
Comprendidos en el rectángulo que aparece en la pantalla hay diez minas ocultas.
Su meta es determinar a través de la intuición, lógica y suerte, donde están
esas minas sin detonar ninguna de ellas. Para hacer ésto, ha de descubrir todos
los cuadrados que no contienen minas; al hacerlo, aprenderá información valiosa
relativa a qué cuadrados contienen minas. Cuidado: descubrir un cuadrado que
contiene una mina la detonará, finalizando la partida inmediatamente.
...
quedado descubiertas. Los cuadrados que contengan un . son aquellos que no están
contiguos a una mina; los números que aparecen en los cuadrados restantes
indican la distancia a la que están de las minas.

Si cree saber donde está una mina, puede “marcar” el cuadrado. Para hacer esto,
seleccione el cuadrado sospechoso y pulse f. Por ejemplo, en la imagen inferior,
decidí que el cuadrado en el lado izquierdo del tablero parecía sospechoso...
...
un “barrido” en torno al cuadrado. Este es solo un conveniente atajo para
descubrir todos los cuadrados cerca de él (exceptuando aquellos marcados,por
supuesto). Por ejemplo, haciendo un barrido en torno al 1 de la imagen superior:
...
Afortunadamente (¿o ha sido suerte?), mi suposición acerca de la ubicación de la
mina era correcta. De haber errado, habría perdido inmediatamente:
...
explotado están marcadas con un símbolo de intercalación (^), y se indica la que
ha “pisado” con un asterisco (*).



--------------------------------------------------------------------------------
[7]Me complace decir que el número de peticiones de esta índole cayeron
dramáticamente a continuación de la primera publicación de esta guía. Sería una
agradable coincidencia de haber una conexión entre ambos eventos.

[8]A esto se le denomina a veces como un “proceso de instalación”, aunque lo que
en realidad esté haciendo sea eliminar o actualizar paquetes, además de
instalarlos.

[9]Como se ha mencionado antes, esto no índica que los paquetes del archivo sean
seguros, o incluso no maliciosos; simplemente muestra que son genuinos.

[10] Más exactamente: se desinstalarán cuando nada conduzca a ellos a través de
«Depende», «Predepende» o «Recomienda» desde un paquete instalado manualmente.
Si APT::AutoRemove::SuggestsImportant es «true», una relación de «Sugiere» es
también suficiente para mantener un paquete instalado.

[11]O cuando la resolución inmediata se desactiva.

[12]El paquete con la más alta prioridad en dpkg, no el paquete con la más alta
prioridad apt pin.

[13]Este límite se impuso porque las estructuras de coste más complejas podían
dificultar la optimización del solucionador. Puede que las versiones futuras del
programa eliminen algunas de las restricciones si resultan innecesarias.

[14] aptitude trata la coma de manera única si hay un segundo argumento, con lo
cual “?name(apt,itude)” busca la cadena “apt,itude” en el campo de Name de los
paquetes.

Aunque este comportamiento está bien definido, puede dar lugar a sorpresas.
Recomiendo usar las comillas dobles para todo patrón que contenga caracteres que
puedan tener algún significado particular.
...
literal, debe usar una barra invertida: “?description(\~|)” muestra paquetes
cuya descripción contiene un carácter de barra vertical (“|”).
...
nombrar de manera explicita la variable en los λ - términos correspondientes al
término. Generalmente, un término tiene la forma “λ x, el nombre es igual a (x,
patrón)”; dando lugar a que un objetivo explicito x sea visible en el lenguaje
de búsqueda.
...
                                                -- Monty Python y el Santo Grial
  ...
  3.3. Quiero seleccionar texto. ¿Porqué aptitude no me permite desactivar el
  ratón?


3.1. ¿Cómo puedo encontrar sólo un paquete en base al nombre?

     Como se menciona en “Patrones de búsqueda”, cuando realiza una búsqueda de un
     paquete en base al nombre, el texto que introduce es en realidad una expresión
     regular. Por ello, el patrón de búsqueda “^nombre$” sólo coincidiría con un paquete
     llamado nombre.

     Por ejemplo, puede buscar apt (pero no aptitude o synaptic) introduciendo ^apt$;
     puede buscar g++ (pero no g++-2.95 o g++-3.0) introduciendo ^g\+\+$.

3.2. ¿Cómo puedo encontrar paquetes rotos?

     Utilice la orden Buscar → Buscar Roto (b).

3.3. Quiero seleccionar texto. ¿Porqué aptitude no me permite desactivar el ratón?

     Normalmente, no puede seleccionar texto empleando el ratón en una terminal xterm
     mientras haya un programa ejecutándose en esa terminal (tal como aptitude). Puede,
     no obstante, invalidar este comportamiento y llevar a cabo una selección
     presionando continuadamente Shift mientras pulsa sobre la terminal.
...
IMG.alt.suffix Nota
  = \[Nota]]
               Esta sección está más bien incompleta, y es probable que se actualice y que
               crezca a medida que pase el tiempo (en particular, muchos de ellos son créditos
               de las traducciones debido al alto número de fuentes de traducciones[21]). Si
               piensa que debería aparecer en esta lista, escríbame a la dirección
               <dburrows@debian.org> con una explicación de porqué piensa que debe estar.
...
Tabla de contenidos
  ...
  aptitude-run-state-bundle — desempaqueta un fichero de estado de aptitude e
  invoca aptitude sobre éste
...
unhold | unmarkauto | versions } paquetes...
...
aptitude [-S nombre_de_fichero] [ --autoclean-on-startup | --clean-on-startup |
-i | -u ]
...
acción que el programa ha de llevar a cabo. Si no se especifica ninguna opción
en la línea de órdenes, aptitude iniciará el modo gráfico.
...
Dispone de las siguientes acciones:
  ...
  install
      ...
      invalidación”:
        ...
        paquete_
            Purga el paquete: lo elimina junto con todos sus ficheros de
            configuración y de datos asociados.
        ...
        paquete:
            Mantiene el paquete en su versión actual: cancela instalar, eliminar
            o actualizar. Al contrario que “retener” (consulte arriba) esto no
            impide actualizaciones automáticas en el futuro.
      ...
      IMG.alt.suffix Nota
        = \[Nota]]
                     Una vez que introduce Y en la petición final de confirmación, la orden
                     “install” modificará la información guardada en aptitude relativa a qué
                     acciones ejecutar. Por ello, si ejecuta la orden, por ejemplo, “aptitude
                     install foo bar” y después interrumpe la instalación durante la descarga e
                     instalación de paquetes, necesitará ejecutar “aptitude remove foo bar” para
                     cancelar esa orden.



  remove, purge, hold, unhold, keep, reinstall
      Estas órdenes realizan lo mismo que “install”, pero en este caso la acción
      nombrada afectaría a todos aquellos paquetes en la línea de órdenes que no
      la invaliden. La diferencia entre hold (retener) y keep (mantener), es que
      el primero causaría que un paquete se ignorase en futuras órdenes safe-
      upgrade o full-upgrade, mientras que keep sólo cancela toda acción
      programada para ese paquete. unhold (anular retención) permitiría
      actualizar un paquete en un futuro con las órdenes safe-upgrade o full-
      upgrade, que de otra forma no alterarían su estado.
  ...
  forbid-version
      Impide que un paquete se actualice a un versión determinada. Esto evita
      que aptitude lo actualice a esa versión de forma automática, pero
      permitirá una actualización automática a otra versión futura. De forma
      predeterminada, aptitude selecciona la versión a la que se actualizaría el
      paquete en cualquier circunstancia; puede invalidar esta selección
      añadiendo “=versión” al nombre del paquete: por ejemplo “aptitude forbid-
      version vim=1.2.3.broken-4”.

      Esta orden es útil para evitar versiones rotas de paquetes sin necesidad
      de definir y eliminar retenciones manuales. Si al final decide que
      realmente quiere la versión prohibida, la orden “aptitude install paquete”
  ...
  safe-upgrade
      ...
      para resolver dependencias a menos que se invoque la orden --no-new-
      installs.
  ...
  versions
      ...
      Para desactivar el nombre de paquete, introduzca --show-package-
      names=never:
  ...
  why, why-not
      ...
      Si hay uno o más patrones, aptitude iniciará la búsqueda a partir de estos
      patrones; esto es, el primer paquete de la cadena que devuelva será un
      paquete que coincide con el patrón en cuestión. Estos patrones se
      consideran como nombres de paquete a menos que contengan un signo de tilde
      (“~”) o un signo de interrogación (“?”), en cuyo caso se toman como
      patrones de búsqueda (consulte la sección “Patrones de búsqueda” en el
      manual de referencia de aptitude).
      ...
      IMG.alt.suffix Nota
        = \[Nota]]
                     aptitude why no ejecuta una resolución completa de dependencias, solo
                     muestra relaciones directas entre paquetes. Por ejemplo, si A depende de B,
                     C depende de D, y B y C entran en conflicto, “aptitude why-not D” no
                     devolvería la respuesta “A depende de B, B entra en conflicto con C, y D
                     depende de C”.
  ...
  download
      Descarga el fichero .deb del paquete dado al directorio actual. Si el
      nombre de un paquete contiene un signo de tilde (“~”) o un signo de
      interrogación (“?”), se tomará como un patrón de búsqueda y se descargarán
      todos los paquetes correspondientes (consulte la sección “Patrones de
      búsqueda” del manual de referencia de aptitude).

      De manera predeterminada, se descarga la versión que se instalaría con
      “aptitude install”. Puede seleccionar una versión en particular de un
      paquete añadiendo =versión al nombre del paquete; puede seleccionar una
      versión de un archivo o distribución en particular añadiendo /archivo o /
      release al nombre del paquete (por ejemplo, /unstable o /sid).
...
estas órdenes las ignorarán.
  ...
  --add-user-tag-to etiqueta,patrón
      ...
      con patrón al igual que con la orden add-user-tag. El patrón es un patrón
      de búsqueda como se describe en la sección “Patrones de búsqueda” en el
      manual de referencia de aptitude.
  ...
  --allow-new-upgrades
      Cuando se utiliza el solucionador seguro (esto es, ha introducido --safe-
      resolver, la opción safe-upgrade, o se define Aptitude::Always-Use-Safe-
      Resolver con valor de true), permite al solucionador de dependencias
      instalar actualizaciónes de paquetes independientemente del valor de
  ...
  --allow-new-installs
      ...
      nuevos. Esta opción tiene efecto sin importar el valor de Aptitude::Safe-
      Resolver::No-New-Installs.
  ...
  -d, --download-only
   ...
   -F formato, --display-format formato
      ...
      Para search, esto se corresponde con la opción de configuración Aptitude::
      CmdLine::Package-Display-Format; para versions, se corresponde con la
      opción de configuración Aptitude::CmdLine::Version-Display-Format.
  ...
  -f
      ...
      Esto se corresponde con la opción de configuración Aptitude::CmdLine::Fix-
      Broken.
  ...
  --full-resolver
   ...
   --group-by modo-de-agrupación
      ...
      * archive para agrupar paquetes según su archivo (“stable”, “unstable”,
        etc). Si un paquete aparece en más de un archivo,se mostrará en cada uno
        de ellos.
      ...
      * none para mostrar todas las versiones en una única lista sin ningún modo
        de agrupación.
      ...
      * source-version para agrupar versiones por su paquete fuente y versión de
        las fuentes.
  ...
  -h, --help
   ...
   --log-file=fichero
      ...
      depuración de fallos. Consulte las opciones en línea de órdenes --log-
      level para controlar qué se registra.
   ...
   --log-level=nivel, --log-level=categoría:nivel
      ...
      configuración más específica es la que tiene efecto. Si introduce --log-
      level=aptitude.resolver:fatal y --log-level=aptitude.resolver.hints.match:
      trace, los mensajes en aptitude.resolver.hints.parse se imprimirán sólo si
      su nivel es fatal, pero aquellos mensajes en aptitude.resolver.hints.match
      se mostrarán. Si configura el nivel de la misma categoría dos o más veces,
      la última configuración es la que tiene efecto.

      No afecta al registro de las instalaciones ejecutadas por aptitude (/var/
      log/aptitude); los mensajes del registro que se escriben con esta
      configuración incluyen programas internos, eventos, errores, y mensajes de
   ...
   --log-resolver
      ...
      --log-level=aptitude.resolver.search:trace --log-
  ...
  --no-new-upgrades
      Cuando se utiliza el solucionador seguro (p. ej., ha introducido --safe-
      resolver o se definió Aptitude::Always-Use-Safe-Resolver con valor de
      true), prohibe al solucionador de dependencias instalar actualizaciones de
      paquetes independientemente del valor de Aptitude::Safe-Resolver::No-New-
      Upgrades.


  --no-show-resolver-actions
      No muestra las acciones ejecutadas por el solucionador “seguro”,
      invalidando toda opción de configuración o una orden anterior --show-
      resolver-actions.
  ...
  --safe-resolver
      ...
      siempre utiliza el solucionador seguro, no acepta la marca --safe-
      resolver.
  ...
  --show-package-names when
      ...
      * auto: muestra los nombres de paquete cuando se ejecuta aptitude versions
        si la salida no está agrupada por paquete, y si hay un argumento de
        búsqueda de patrón o si hay más de un argumento.

      * never: nunca muestra nombres de paquete en la salida de aptitude
        versions.


      Esta opción se corresponde con la opción de configuración Aptitude::
      CmdLine::Versions-Show-Package-Names.


  --show-resolver-actions
      Muestra las acciones ejecutadas por el solucionador “seguro”, y por safe-
      upgrade.

      Al ejecutar la orden safe-upgrade o cuando se ha introduce la opción --
      safe-resolver, aptitude muestra un resumen de las acciones tomadas por el
      solucionador antes de mstrar la previsualización de la instalación. Esto
      equivale a la opción de configuración Aptitude::Safe-Resolver::Show-
      Resolver-Actions.
  ...
  --show-summary[=MODO]
      ...
      contengan dependencias del tipo «Sugiere» no se mostrarán: combine --show-
      summary con -v para ver un resumen de todas las razones por las que el
      paquete objetivo se va a instalar.

      El MODO puede ser cualquiera de los siguientes:


        1. no-summary: no muestra el resumen (el comportamiento predeterminado
  ...
  -t distribución, --target-release distribución
      Define la rama de la que se deberían instalar los paquetes. Por ejemplo,
      “aptitude -t experimental ...” instalaría paquetes de la distribución
      «experimental» a menos que especifique lo contrario. Para las acciones en
      línea de órdenes “changelog”, “download”, y “show”, esto equivale a añadir
      /rama al final de cada paquete nombrado en la línea de órdenes; para otras
      órdenes, esto afecta a la versión candidata de los paquetes de acuerdo a
  ...
  -w ancho, --width ancho
      Define el ancho de pantalla que utilizar para la salida de la orden search
      (el ancho de la terminal se utiliza de manera predeterminada).
...
diseñado para un uso interno; generalmente, no tendrá que utilizarlas.
  ...
  --autoclean-on-startup
      Elimina los ficheros descargados y antiguos al inicio (equivale a iniciar
      el programa y seleccionar inmediatamente Acciones → Limpiar el almacén de
      paquetes). No puede usar esta opción y “--autoclean-on-startup”, “-i”, o
      “-u” a la vez.
...
La orden aptitude-create-state-bundle genera un fichero comprimido que guarda
los ficheros necesarios para mimetizar el estado actual del archivo de paquetes.
Se incluyen en el archivo generado los siguientes ficheros y directorios:
...
El fichero de estado es simplemente un fichero tar(1) comprimido con bzip2(1) o
...
continuación. Si no se introduce programa, su valor por omisión es aptitude(8).


Opciones

Las siguientes opciones pueden preceder al fichero de entrada en la línea de
órdenes. Las opciones a continuación del fichero de entrada se toman como
